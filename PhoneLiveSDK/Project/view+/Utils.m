...
+ (NSDictionary *)emojiDict
{
    ...
    dispatch_once(&once, ^ {
        NSBundle *bundle = XResourceBundle;
...
+ (NSAttributedString *)emojiStringFromAttrString:(NSAttributedString*)attrString
{
    ...
    for (NSTextCheckingResult *match in resultsArray) {
        ...
        if ([emojiName hasPrefix:@"["] && emoji[emojiName]) {
            ...
            textAttachment.image = [ImageBundle imagewithBundleName:emoji[emojiName]];
            ...
            if (emoji[emojiName]) {
                ...
                UIImage *emojiImage = [ImageBundle imagewithBundleName:[emojiName stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@":"]]];
                NSTextAttachment *textAttachment = [NSTextAttachment new];
