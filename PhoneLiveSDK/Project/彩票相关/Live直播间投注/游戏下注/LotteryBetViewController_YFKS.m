...
#define heightView LotteryWindowNewHeighYFKS

@interface LotteryBetViewController_YFKS (){
    BOOL isShow;
...
@implementation LotteryBetViewController_YFKS
        ...
        if(_isExit){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [UIView animateWithDuration:0.3 animations:^{
                    ...
                    strongSelf.contentView.bottom = _window_height;
...
- (void)viewDidLoad {
    ...
    NSArray<UIView*> *betViews = @[self.bet_big_View,self.bet_small_View,self.bet_single_View,self.bet_double_View,self.bet_bao1_View,self.bet_bao6_View];
...
//    [self startWobble];
    ...
    self.betHistoryList.alwaysBounceVertical = YES;
}
    ...
    if(curLotteryType>0 && [lotteryType isEqualToString:[NSString stringWithFormat:@"%ld",curLotteryType]]){
        ...
        for (int i = 0; i<subViews.count; i++) {
            ...
            if (sub.tag >=20000 && sub.tag<=888888) {
                ...
                for (UIView *subV in winViews) {
                    CGPoint originInSubV = [subV convertPoint:sub.frame.origin fromView:sub.superview];
                    if ([subV.layer containsPoint:originInSubV]) {
                        isContained = YES;
                        break;
                    }
                }
        ...
        [UIView animateWithDuration:0.8 animations:^{
            for (UIView *sub in chipLose) {
                sub.center = self.shaiguImgView.center;
            ...
            for (UIView *sub in chipLose) {
                    NSLog(@"sub.center after animation: %@", NSStringFromCGPoint(sub.center));
                }
        ...
        [UIView animateWithDuration:0.8 delay:1 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            if (winViews.count>1) {
                for (int i = 0;i<chipLose.count;i++) {
                    ...
                    [sub mas_updateConstraints:^(MASConstraintMaker *make) {
                        make.left.mas_equalTo(sub.left);
                        make.bottom.equalTo(strongSelf.contentView.mas_bottom).offset(-(strongSelf.contentView.height-sub.bottom));
                    }];
                }
            }else{
                for (UIView *sub in chipLose) {
        ...
        [UIView animateWithDuration:0.8 delay:2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            if (strongSelf->winUsersArray.count>0) {
                ...
                for (LotteryProfit *betWinModel in strongSelf->winUsersArray) {
                    ...
                    if ([winWaysStr isEqualToString:@"总和_大"]||[winWaysStr isEqualToString:@"总和_小"]||[winWaysStr isEqualToString:@"总和_单"]||[winWaysStr isEqualToString:@"总和_双"]||[winWaysStr isEqualToString:@"豹子_1"]||[winWaysStr isEqualToString:@"豹子_6"]) {
                ...
                if (dicWinWays.count>0) {
                    ...
                    for (int i = 0; i<winWaysStrs.count; i++) {
                        ...
                        if ([winSubWay isEqualToString:@"总和_大"]) {
                            tagChipsub = 20000;
                        }else if([winSubWay isEqualToString:@"总和_小"]){
                            tagChipsub = 20001;
                        }else if([winSubWay isEqualToString:@"总和_单"]){
                            tagChipsub = 20002;
                        }else if([winSubWay isEqualToString:@"总和_双"]){
                            tagChipsub = 20003;
                        }else if([winSubWay isEqualToString:@"豹子_1"]){
                            tagChipsub = 20004;
                        }else if([winSubWay isEqualToString:@"豹子_6"]){
                            tagChipsub = 20005;
                        }
...
-(void)betWintification:(NSNotification*)notification
{
    ...
    if ([betModel isKindOfClass: [LotteryBarrage class]]) {
        return;
    }
...
-(void)betNotification:(NSNotification*)notification
{
    ...
    if (betModel.msg.issue == [self.curIssue longLongValue] && curLotteryType == betModel.msg.lotteryType) {
        ...
        for (int i = 0; i<arrWays.count; i++) {
            ...
            }else if ([subWay isEqualToString:@"豹子_1"]){
                keyContinue = @"5";
            ...
            }else if ([subWay isEqualToString:@"豹子_6"]){
                keyContinue = @"6";
...
//    if (uid == [[Config getOwnID] longLongValue]) {
...
//            [bubble setBetCount:totalMoney];
//        }else{
//            [bubble setBetCount:[NSString stringWithFormat:@"%ld",([bubble.betNum integerValue]+[totalMoney integerValue])]];
//        }
...
//        (18 -7; 62 69)
        ...
        if (!gaiziImg) {
            gaiziImg = [[UIImageView alloc] initWithFrame:CGRectMake(2, -32, 62, 69)];
            gaiziImg.image = [ImageBundle imagewithBundleName:@"yfks_gaizi"];
            gaiziImg.tag = 1200;
            gaiziImg.alpha = 0;
            [self.shaiguImgView addSubview:gaiziImg];
        }
        UIView *resultShaigu1 = [self.shaiguImgView viewWithTag:1300];
        
        [self.shaiguImgView bringSubviewToFront:gaiziImg];
        WeakSelf
        [UIView animateWithDuration:0.5 animations:^{
            gaiziImg.frame = CGRectMake(12.6, -5, 43.4, 48.3);
            gaiziImg.alpha = 1;
            resultShaigu1.alpha = 0;
        } completion:^(BOOL finished) {
            STRONGSELF
            if (strongSelf == nil) {
                return;
            ...
            self->animationShaigu.alpha = 1;
            self->animationShaigu.hidden = YES;
...
-(void)begainAnimationShaigu
{
    if (!begainAnimationShaigu) {
        ...
        UIImageView *gaiziImg = [self.shaiguImgView viewWithTag:1200];
        if (!gaiziImg) {
            gaiziImg = [[UIImageView alloc] initWithFrame:CGRectMake(12.6, -5, 43.4, 48.3)];
            gaiziImg.image = [ImageBundle imagewithBundleName:@"yfks_gaizi"];
            gaiziImg.tag = 1200;
            [self.shaiguImgView addSubview:gaiziImg];
        }
        gaiziImg.alpha = 1;
        gaiziImg.frame = CGRectMake(12.6, -5, 43.4, 48.3);
        animationShaigu.alpha = 0;
        animationShaigu.hidden = NO;
        WeakSelf
        [UIView animateWithDuration:0.5 animations:^{
            gaiziImg.frame = CGRectMake(0, -30, 43.4, 48.3);
            gaiziImg.alpha = 0;
            self->animationShaigu.alpha = 1;
        } completion:^(BOOL finished) {
            STRONGSELF
            ...
            if (strongSelf->animationShaigu == nil) {
                ...
                strongSelf->animationShaigu = [[YYAnimatedImageView alloc] initWithFrame:CGRectMake(10, 0, 45, 44)];
...
-(void)closeAnimationAndShowResult:(NSString*)result winways:(NSArray*)winways
{
    ...
    [UIView animateWithDuration:0.1 animations:^{
        self->animationShaigu.alpha = 0;
    
    }];
    UIView *resultView = [[UIView alloc]initWithFrame:CGRectMake(8, 0,45,42)];
    resultView.tag = 1300;
    [self.shaiguImgView addSubview:resultView];
    NSArray *arrayResult = [result componentsSeparatedByString:@","];
    UIImageView *img1 = [[UIImageView alloc]initWithFrame:CGRectMake(14.4, 10.8, 10.8, 10.8)];
    img1.image = [ImageBundle imagewithBundleName:[NSString stringWithFormat:@"shaizi_result_%@",arrayResult[0]]];
    UIImageView *img2 = [[UIImageView alloc]initWithFrame:CGRectMake(25.2, 10.8, 10.8, 10.8)];
    img2.image = [ImageBundle imagewithBundleName:[NSString stringWithFormat:@"shaizi_result_%@",arrayResult[1]]];
    UIImageView *img3 = [[UIImageView alloc]initWithFrame:CGRectMake(21, 16.8, 10.8, 10.8)];
    ...
    resultPopView.center = CGPointMake(SCREEN_WIDTH/2, (390)/2.0);
    [resultPopView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.contentView.mas_bottom).offset(-(heightView+ShowDiff)/2.0+74/2);   // 设置y坐标偏移
        // 如果需要设置lotterySubBetV的宽度和高度，可以使用下面的代码
    ...
    for (int i=0; i<maxCount; i++) {
        ...
        if (name&&[name isEqualToString:@"猜大小"]) {
            ...
            for (int y = 0; y<options.count; y++) {
                ...
                NSString *title = [subWayDic objectForKey:@"title"];
                NSString *titleSt = wayInfo[@"st"];
                NSString *titleSt1 = [subWayDic objectForKey:@"st"];
                if ([winways containsObject:title]) {
                    winStr = [NSString stringWithFormat:@"%@ %@",winStr,[NSString stringWithFormat:@"%@%@",titleSt,titleSt1]];
                }
            }
        }
    }
...
-(void)initUI{
    ...
    historyMaskView.layer.cornerRadius = 8;
...
- (void)clickHistoryBetAction:(BetListDataModel *)model {
    ...
    [model.detail.way enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *keyContinue = @"";
        if ([obj isEqualToString:@"总和_大"]) {
            keyContinue = @"1";
        }else if ([obj isEqualToString:@"总和_小"]){
            keyContinue = @"2";
        }else if ([obj isEqualToString:@"总和_单"]){
            keyContinue = @"3";
        }else if ([obj isEqualToString:@"总和_双"]){
            keyContinue = @"4";
        }else if ([obj isEqualToString:@"豹子_1"]){
            keyContinue = @"5";
        }else if ([obj isEqualToString:@"豹子_6"]){
            keyContinue = @"6";
        }
        ...
        if (name&&[name isEqualToString:@"猜大小"]) {
            ...
            for (int y = 0; y<options.count; y++) {
                ...
                if (title&&[title isEqualToString:@"豹子_1"]) {
                    ...
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = [NSString stringWithFormat:@"%@%@",titleSt,titleSt1];
                    valueLabel.text = [@"x" stringByAppendingString:value];
                ...
                if (betmine> 0) {
                    LotteryBetSubView *lotterySubBetV = [self.contentView viewWithTag:toView.tag+888888];
                    if (!lotterySubBetV) {
                        WeakSelf
                        lotterySubBetV = [LotteryBetSubView instanceLotteryBetSubViewwWithFrame:CGRectMake(toView.frame.origin.x+20, toView.frame.origin.y-10, toView.width-40, (toView.width-40)/80*60) contentEdge:6 withBlock:^(BOOL sure) {
                            STRONGSELF
                            if (strongSelf == nil) {
                                return;
                            }
                        ...
                        [lotterySubBetV mas_makeConstraints:^(MASConstraintMaker *make) {
                            // 这里假设你想要设置lotterySubBetV的左上角坐标（x，y）相对于self.contentView的左上角
                            make.left.equalTo(self.contentView.mas_left).offset(lotterySubBetV.x); // 设置x坐标偏移
                            make.bottom.equalTo(self.contentView.mas_bottom).offset(-(self.contentView.height-lotterySubBetV.bottom));   // 设置y坐标偏移
                            // 如果需要设置lotterySubBetV的宽度和高度，可以使用下面的代码
                             make.width.equalTo(@(lotterySubBetV.width)); // 设置宽度
                             make.height.equalTo(@(lotterySubBetV.height)); // 设置高度
                        }];
                        [lotterySubBetV updateMineNumb:betmine];
                    }else{
                        [lotterySubBetV updateMineNumb:betmine];
...
-(void)doBetType:(NSString*)detailType chipNum:(NSInteger)chipNum toView:(UIView*)toView {
    ...
    }else if ([detailType isEqualToString:@"豹子_1"]){
    ...
    }else if ([detailType isEqualToString:@"豹子_6"]){
...
//        [MBProgressHUD showError:YZMsg(@"LobbyBet_selecte_Warning")];
    ...
    WeakSelf
    NSString *betUrl = [NSString stringWithFormat:@"Lottery.Betting&uid=%@&token=%@&lottery_type=%@&money=%@&way=%@&serTime=%@&issue=%@&optionName=%@&liveuid=%@&betid=%@",[Config getOwnID],[Config getOwnToken],lottery_type,money,way,minnum([[NSDate date] timeIntervalSince1970]),issue,@"猜大小",liveuidstr,betIdStr];//User.getPlats
...
//            [MBProgressHUD showError:msg];
            // 清空信息
            NSArray<UIView*> *betViews = @[strongSelf.bet_big_View,strongSelf.bet_small_View,strongSelf.bet_single_View,strongSelf.bet_double_View,strongSelf.bet_bao1_View,strongSelf.bet_bao6_View];
            for (int i = 0; i<betViews.count; i++) {
...
-(void)exitView{
    ...
    self.bottomConstraint.constant =  -(heightView+[self LobbyWindowHeight]);
...
//        [btn setTitle:YZMsg(@"trend_title") forState:UIControlStateSelected];
        [UIView animateWithDuration:0.3 animations:^{
            [btn setTitle:YZMsg(@"trend_title") forState:UIControlStateSelected];
            [btn sizeToFit];
            self.toolScrollView.contentSize = CGSizeMake(35*self.toolBtnArr.count + (btn.width-35), 0);
            for (int i = 0; i< self.toolBtnArr.count; i ++) {
                UIButton *livechatBtn = self.toolBtnArr[i];
                if(i == 0){
                    livechatBtn.frame = CGRectMake(5,5,btn.width,30);
                }else{
                    livechatBtn.frame = CGRectMake(5+(btn.width-30)+i*(30+5),5,30,30);
                }
            }
            [self.contentView layoutIfNeeded];
        } completion:^(BOOL finished) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [btn sizeToFit];
            self.toolScrollView.contentSize = CGSizeMake(35*self.toolBtnArr.count + (btn.width-35), self.toolScrollView.height);
            for (int i = 0; i< self.toolBtnArr.count; i ++) {
                UIButton *livechatBtn = self.toolBtnArr[i];
                if(i == 0){
                    livechatBtn.frame = CGRectMake(5,5,30,30);
                }else if(i == 1){
                    livechatBtn.frame = CGRectMake(5+i*(30+5),5,btn.width,30);
                }else{
                    livechatBtn.frame = CGRectMake(5+(btn.width-30)+i*(30+5),5,30,30);
                }
            }
...
//            VC.hightRate = 0.5;
            ...
            VC.urls = url;
            WeakSelf
            UIView *shadowView1 = [YBToolClass mengban:self.view clickCallback:^{
                STRONGSELF
                if (strongSelf == nil) {
                    return;
                }
            ...
            VC.closeCallback = ^{
                ...
                [shadowView1 removeFromSuperview];
...
-(void)rebackScrollView{
    ...
    isShowTopList = NO;
    self.noView.hidden = YES;
    self.toolScrollView.contentSize = CGSizeMake(35*self.toolBtnArr.count + 10, self.toolScrollView.height);
    [UIView animateWithDuration:0.3 animations:^{
        for (int i = 0; i< self.toolBtnArr.count; i ++) {
            UIButton *livechatBtn = self.toolBtnArr[i];
            livechatBtn.frame = CGRectMake(5+i*(30+5),5,30,30);
