...
#define heightView LotteryWindowNewHeighLH


@interface LotteryBetViewController_LH(){
    BOOL isShow;
...
@implementation LotteryBetViewController_LH
...
- (void)viewDidLoad {
    ...
    [betContentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(-self.bottomimgConstraint.constant-60);
    }];
    ...
    NSArray<UIView*> *betViews = @[self.bet_player_View,self.bet_equal_View,self.bet_bank_View];
    ...
    [self.dragan_title_imgV vk_slantHorizontal:-5];
    [self.tiger_title_imgV vk_slantHorizontal:5];
    ...
    [UIView animateWithDuration:0.8 delay:2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (strongSelf->winUsersArray.count>0) {
            ...
            for (LotteryProfit *betWinModel in strongSelf->winUsersArray) {
                ...
                //0-0
                if ([winWaysStr isEqualToString:@"龙虎_虎"]||[winWaysStr isEqualToString:@"龙虎_和"]||[winWaysStr isEqualToString:@"龙虎_龙"]) {
            ...
            if (dicWinWays.count>0) {
                ...
                for (int i = 0; i<winWaysStrs.count; i++) {
                    ...
                    //0-0
                    if ([winSubWay isEqualToString:@"龙虎_龙"]) {
                        tagChipsub = 20000;
                    }else if([winSubWay isEqualToString:@"龙虎_和"]){
                        tagChipsub = 20002;
                    }else if([winSubWay isEqualToString:@"龙虎_虎"]){
                        tagChipsub = 20004;
                    }
...
//        }
    ...
    */
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        UILabel *labelTip = [[UILabel alloc]initWithFrame: CGRectMake(0, 30, 175, 15)];
        ...
        for (int i=0; i<maxCount; i++) {
            ...
            if (name&&[name isEqualToString:@"龙虎"]) {
                for (int x = 0; x<options.count; x++) {
                    NSDictionary *subWayDic = options[x];
                    NSString *titleSub = subWayDic[@"title"];
                    if ([titleSub isEqualToString:@"龙虎_龙"]||[titleSub isEqualToString:@"龙虎_和"]||[titleSub isEqualToString:@"龙虎_虎"]) {
                        NSString *title = [subWayDic objectForKey:@"title"];
                        NSString *titleSt = [subWayDic objectForKey:@"st"];
        ...
        UILabel *labelTip2 = [[UILabel alloc]initWithFrame: CGRectMake(175, 30, 175, 15)];
...
- (void)clickHistoryBetAction:(BetListDataModel *)model {
    ...
    [model.detail.way enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *keyContinue = @"";
        if ([obj isEqualToString:@"龙虎_龙"]) {
            keyContinue = @"1";
        }else if ([obj isEqualToString:@"龙虎_和"]){
            keyContinue = @"2";
        }else if ([obj isEqualToString:@"龙虎_虎"]){
            keyContinue = @"3";
        }
...
//    
    ...
    UINib *copennib=[UINib nibWithNibName:kLotteryOpenViewCell_LH bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
    [self.openResultList registerNib: copennib forCellWithReuseIdentifier:kLotteryOpenViewCell_LH];
...
-(void)doBetNetwork{
    ...
    WeakSelf
    NSString *betUrl = [NSString stringWithFormat:@"Lottery.Betting&uid=%@&token=%@&lottery_type=%@&money=%@&way=%@&serTime=%@&issue=%@&optionName=%@&liveuid=%@&betid=%@",[Config getOwnID],[Config getOwnToken],lottery_type,money,way,minnum([[NSDate date] timeIntervalSince1970]),issue,@"龙虎",liveuidstr,betIdStr];//User.getPlats
    [[YBNetworking sharedManager] postNetworkWithUrl:betUrl withBaseDomian:YES andParameter:nil data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if(code == 0)
        {
            ...
            // 清空信息
            NSArray<UIView*> *betViews = @[strongSelf.bet_player_View,strongSelf.bet_equal_View,strongSelf.bet_bank_View];
            for (int i = 0; i<betViews.count; i++) {
