...
@implementation LotteryMenuBarView
...
- (VKBaseCollectionView *)recordTableView {
    if (!_recordTableView) {
        ...
        _recordTableView.clickCellActionBlock = ^(NSIndexPath *indexPath, id item, NSInteger actionIndex) {
            _strongify(self)
            VKBLOCK(self.clickContinueBlock, item);
        };
    }
...
- (VKBaseTableView *)resultTableView {
    if (!_resultTableView) {
        ...
        _resultTableView.automaticDimension = YES;
        ...
        _resultTableView.loadDataBlock = ^{
            ...
            [LotteryNetworkUtil getOpenHistory:self.lotteryType page:self.resultTableView.pageIndex block:^(NetworkData *networkData) {
                ...
                [array setValue:@(self.lotteryType) forKeyPath:@"lotteryType"];
...
- (void)clickM1Action:(UIButton *)sender {
    if (self.canClosed && sender.selected) {
        [self hideAllView];
        [self resetView];
        return;
    }
...
//    [vc showFromBottom];
    ...
    WeakSelf
    UIView *shadowView = [YBToolClass mengban:vkTopVC().view clickCallback:^{
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
    ...
    [vkTopVC().view addSubview:vc.view];
    [vkTopVC() addChildViewController:vc];
}
...
- (void)clickM7Action:(UIButton *)sender {
    ...
    LotterySwitchSet(self.lotteryType, YES);
}
...
- (void)refreshBetRecord {
    if (!self.recordTableView.hidden) {
        [self.recordTableView vk_headerBeginRefresh];
    }
}
