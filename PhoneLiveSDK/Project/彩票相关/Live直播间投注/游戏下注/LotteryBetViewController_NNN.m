//
//  LotteryBetViewController_NNN.m
//  phonelive2
//
...
#define heightView LotteryWindowNewNNHeigh

@interface LotteryBetViewController_NNN(){
    BOOL isShow;
...
@implementation LotteryBetViewController_NNN
...
- (void)viewDidLoad {
    ...
    self.result_blue_View = [LotteryResultView_NN new];
    self.result_blue_View.type = NNLotteryResultTypeLeft;
    self.result_blue_View.startX = 85;
    
    self.result_red_View = [LotteryResultView_NN new];
    self.result_red_View.type = NNLotteryResultTypeRight;
    self.result_red_View.startX = -25;
    ...
    betStackView.alignment = UIStackViewAlignmentCenter;
    ...
    UIStackView *stackView0 = [UIStackView new];
    stackView0.spacing = 5;
    stackView0.transform = CGAffineTransformMakeScale(1.0, 1.4);
    stackView0.distribution = UIStackViewDistributionFillEqually;
    [stackView0 addArrangedSubview:self.result_blue_View];
    [stackView0 addArrangedSubview:self.result_red_View];
    [betStackView addArrangedSubview:stackView0];
    [stackView0 mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [stackView1 addArrangedSubview:self.bet_playerDouble_View];
    ...
    [stackView2 addArrangedSubview:self.bet_equal_View];
    [stackView2 addArrangedSubview:self.bet_superSix_View];
    [stackView2 addArrangedSubview:self.bet_bank_View];
    ...
    [betStackView setCustomSpacing:26 afterView:stackView0];
...
- (void)setLotteryType:(NSInteger)lotteryType{
    ...
    [GameToolClass setCurOpenedLotteryType:-1];
}
//获取彩种信息
- (void)getInfo{
...
- (void)refreshLastOpen:(NSNotification *)notification {
    ...
    if(curLotteryType>0 && [lotteryType isEqualToString:[NSString stringWithFormat:@"%ld",curLotteryType]]){
        ...
        //关闭骰子动画显示结果
//        ---- 开牌 ----
        NSDictionary *bjlNum = notification.userInfo[@"niu"];
    ...
    [UIView animateWithDuration:0.8 delay:2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (strongSelf->winUsersArray.count>0) {
            ...
            for (LotteryProfit *betWinModel in strongSelf->winUsersArray) {
                ...
                //0-0
                if ([winWaysStr isEqualToString:@"蓝方胜"]||[winWaysStr isEqualToString:@"红方胜"]||[winWaysStr isEqualToString:@"蓝方_无牛"]||[winWaysStr isEqualToString:@"蓝方_牛牛"]||[winWaysStr isEqualToString:@"红方_无牛"]||[winWaysStr isEqualToString:@"红方_牛牛"]) {
            ...
            if (dicWinWays.count>0) {
                ...
                for (int i = 0; i<winWaysStrs.count; i++) {
                    ...
                    //0-0
                    if ([winSubWay isEqualToString:@"蓝方胜"]) {
                        tagChipsub = 20000;
                    }else if([winSubWay isEqualToString:@"红方胜"]){
                        tagChipsub = 20001;
                    }else if([winSubWay isEqualToString:@"蓝方_无牛"]){
                        tagChipsub = 20002;
                    }else if([winSubWay isEqualToString:@"蓝方_牛牛"]){
                        tagChipsub = 20003;
                    }else if([winSubWay isEqualToString:@"红方_无牛"]){
                        tagChipsub = 20004;
                    }else if([winSubWay isEqualToString:@"红方_牛牛"]){
                        tagChipsub = 20005;
                    }
            ...
            if (seconds<=0) {
                ...
                dispatch_main_async_safe(^{
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        ...
                        [strongSelf.result_blue_View showPockerAnimation];
                        [strongSelf.result_red_View showPockerAnimation];
                    });
                });
...
-(void)betNotification:(NSNotification*)notification
{
    ...
    if (betModel.msg.issue == [self.curIssue longLongValue]) {
        ...
        for (int i = 0; i<arrWays.count; i++) {
            ...
            }else if ([subWay isEqualToString:@"红方胜"]){
                keyContinue = @"2";
            ...
            }else if ([subWay isEqualToString:@"蓝方_无牛"]){
                keyContinue = @"3";
            ...
            }else if ([subWay isEqualToString:@"蓝方_牛牛"]){
                keyContinue = @"4";
            ...
            }else if ([subWay isEqualToString:@"红方_无牛"]){
                keyContinue = @"5";
            ...
            }else if ([subWay isEqualToString:@"红方_牛牛"]){
                keyContinue = @"6";
...
-(void)refreshTimeUI{
    if(betLeftTime > 0){
        //倒计时中 倒计时动画
        isClosed = (betLeftTime <= sealingTime);
        int beginTime = (betLeftTime - sealingTime) % 60;
        int closeTime = betLeftTime % 60;
        int seconds = isClosed ? closeTime : beginTime;
        if (!isFinance && !_isExit) {
            ...
            UILabel *labelTip = [willReadyView viewWithTag:101];
            if (labelTip) {
                labelTip.text = isClosed ? YZMsg(@"LobbyLotteryVC_betEnd") : YZMsg(@"game_begain");
            }
            if (seconds<5) {
                [buttonick shakeWithOptions:SCShakeOptionsDirectionRotate | SCShakeOptionsForceInterpolationLinearDown | SCShakeOptionsAtEndComplete force:0.15 duration:0.55 iterationDuration:0.05 completionHandler:^{
                }];
...
-(void)closeShaiguBoxAnimation
{
    ...
    if (!closeAnimationShaigu) {
        ...
        [self.result_blue_View hidePockerAnimation];
        [self.result_red_View hidePockerAnimation];
    }
}
...
- (NSArray *)analyzePokerResultWithResult:(NSString *)result{
    if (result.length > 0) {
        ...
        if (result_arr.count == 2) {
            NSString *player = [result_arr.firstObject substringFromIndex:3];
            NSString *banker = [result_arr.lastObject substringFromIndex:3];
...
-(void)closeAnimationAndShowResult:(NSString*)result winways:(NSArray*)winways andVSNum:(NSDictionary *)numResult
{
    ...
    BOOL isBlueWin = [winways containsObject:YZMsg(@"OpenAward_NiuNiu_BlueWin")];
    
    NSArray *result_arr = [self analyzePokerResultWithResult:result];
    self.result_blue_View.pockers = result_arr.firstObject;
    self.result_blue_View.result = numResult[@"blue_niu"];
    self.result_blue_View.selected = isBlueWin;
    [self.result_blue_View openPockerAnimation];
    
    self.result_red_View.pockers = result_arr.lastObject;
    self.result_red_View.result = numResult[@"red_niu"];
    self.result_red_View.selected = !isBlueWin;
    [self.result_red_View openPockerAnimation];
...
//        ///悬浮开奖结果
//        UIView *resultPopView = [UIView new];
//        resultPopView.layer.cornerRadius = 10;
//        resultPopView.layer.borderColor = vkColorRGB(248, 194, 66).CGColor;
//        resultPopView.layer.borderWidth = 3;
//        resultPopView.backgroundColor = vkColorRGBA(0, 0, 0, 0.5);
...
//        }];
//
//        LotteryResultView_NN *blueResultView = [LotteryResultView_NN new];
//        blueResultView.type = NNLotteryResultTypeLeft;
//        [resultPopView addSubview:blueResultView];
//        [blueResultView mas_makeConstraints:^(MASConstraintMaker *make) {
...
//        }];
//
//        LotteryResultView_NN *redResultView = [LotteryResultView_NN new];
//        redResultView.type = NNLotteryResultTypeRight;
//        [resultPopView addSubview:redResultView];
//        [redResultView mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.right.mas_equalTo(-10);
//            make.centerY.mas_equalTo(0);
//            make.height.width.mas_equalTo(blueResultView);
//            make.left.mas_equalTo(blueResultView.mas_right).offset(10);
//        }];
//
//        [resultPopView layoutIfNeeded];
//        [blueResultView setupPockerView];
//        [redResultView setupPockerView];
//
//        blueResultView.result = numResult[@"blue_niu"];
//        blueResultView.pockers = result_arr.firstObject;
//        blueResultView.selected = isBlueWin;
//        [blueResultView openPockerAnimation];
//
//        redResultView.result = numResult[@"red_niu"];
//        redResultView.pockers = result_arr.lastObject;
//        redResultView.selected = !isBlueWin;
//        [redResultView openPockerAnimation];
    });
}
...
- (void)clickHistoryBetAction:(BetListDataModel *)model {
    ...
    [model.detail.way enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *keyContinue = @"";
        if ([obj isEqualToString:@"蓝方胜"]) {
            keyContinue = @"1";
        }else if ([obj isEqualToString:@"红方胜"]){
            keyContinue = @"2";
        }else if ([obj isEqualToString:@"蓝方_无牛"]){
            keyContinue = @"3";
        }else if ([obj isEqualToString:@"蓝方_牛牛"]){
            keyContinue = @"4";
        }else if ([obj isEqualToString:@"红方_无牛"]){
            keyContinue = @"5";
        }else if ([obj isEqualToString:@"红方_牛牛"]){
            keyContinue = @"6";
        }
...
-(void)initDeskInfo{
    ...
    for (int i=0; i<maxCount; i++) {
        ...
        if (name && [name isEqualToString:@"猜胜负"]) {
            for (int x = 0; x<options.count; x++) {
                ...
                //0-0
                if (title&&[title isEqualToString:@"蓝方胜"]) {
                ...
                }else if(title&&[title isEqualToString:@"红方胜"]){
        ...
        } else if (name && [name isEqualToString:@"猜蓝方牛"]) {
            for (int x = 0; x<options.count; x++) {
                NSDictionary *subWayDic = options[x];
                NSString *title = [subWayDic objectForKey:@"title"];
                NSString *titleSt = [subWayDic objectForKey:@"st"];
                ...
                UIView *toView = nil;
                NSInteger tagChipN = 0;
                titleSt = [NSString stringWithFormat:@"%@%@", [YZMsg(@"PublicObj_blue") stringByReplacingOccurrencesOfString:@":" withString:@""], titleSt];
                //0-0
                if (title&&[title isEqualToString:@"蓝方_无牛"]) {
                ...
                }else if(title&&[title isEqualToString:@"蓝方_牛牛"]){
                    ...
                    //UILabel *betMineLabel = [self.bet_superSix_View viewWithTag:101];
        ...
        } else if (name && [name isEqualToString:@"猜红方牛"]) {
            ...
            for (int x = 0; x<options.count; x++) {
                ...
                UIView *toView = nil;
                NSInteger tagChipN = 0;
                titleSt = [NSString stringWithFormat:@"%@%@", [YZMsg(@"PublicObj_red") stringByReplacingOccurrencesOfString:@":" withString:@""], titleSt];
                //0-0
                if (title&&[title isEqualToString:@"红方_无牛"]) {
                ...
                }else if(title&&[title isEqualToString:@"红方_牛牛"]){
                    ...
                    //UILabel *betMineLabel = [self.bet_bankDouble_View viewWithTag:101];
    ...
    [self.result_blue_View setupPockerView];
    [self.result_red_View setupPockerView];
}
...
-(void)betWithView:(UIView *)tapView chipNum:(NSInteger)chipNum
{
    ...
    if ([tapView isEqual:self.bet_player_View]) {
        //0-0
        [self doBetType:@"蓝方胜" chipNum:chipNum toView:self.bet_player_View];
    }else if ([tapView isEqual:self.bet_playerDouble_View]){
        [self doBetType:@"红方胜" chipNum:chipNum toView:self.bet_playerDouble_View];
    }else if ([tapView isEqual:self.bet_equal_View]){
        [self doBetType:@"蓝方_无牛" chipNum:chipNum toView:self.bet_equal_View];
    }else if ([tapView isEqual:self.bet_superSix_View]){
        [self doBetType:@"蓝方_牛牛"chipNum:chipNum toView:self.bet_superSix_View];
    }else if ([tapView isEqual:self.bet_bank_View]){
        [self doBetType:@"红方_无牛" chipNum:chipNum toView:self.bet_bank_View];
    }else if ([tapView isEqual:self.bet_bankDouble_View]){
        [self doBetType:@"红方_牛牛" chipNum:chipNum toView:self.bet_bankDouble_View];
    }
}
...
-(void)doBetType:(NSString*)detailType chipNum:(NSInteger)chipNum toView:(UIView*)toView{
    ...
    if ([detailType isEqualToString:@"蓝方胜"]) {
    ...
    }else if ([detailType isEqualToString:@"红方胜"]){
    ...
    }else if ([detailType isEqualToString:@"蓝方_无牛"]){
    ...
    }else if ([detailType isEqualToString:@"蓝方_牛牛"]){
    ...
    }else if ([detailType isEqualToString:@"红方_无牛"]){
    ...
    }else if ([detailType isEqualToString:@"红方_牛牛"]){
...
//        [MBProgressHUD showError:YZMsg(@"LobbyBet_selecte_Warning")];
    ...
    WeakSelf
    NSString *betUrl = [NSString stringWithFormat:@"Lottery.Betting&uid=%@&token=%@&lottery_type=%@&money=%@&way=%@&serTime=%@&issue=%@&optionName=%@&liveuid=%@&betid=%@",[Config getOwnID],[Config getOwnToken],lottery_type,money,way,minnum([[NSDate date] timeIntervalSince1970]),issue,@"百人牛牛",liveuidstr,betIdStr];//User.getPlats
