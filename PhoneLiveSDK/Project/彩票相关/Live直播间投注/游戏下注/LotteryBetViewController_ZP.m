...
#define heightView LotteryWindowNewHeighZP

@interface LotteryBetViewController_ZP ()<CAAnimationDelegate>{
    BOOL isShow;
...
@implementation LotteryBetViewController_ZP
...
- (void)viewDidLoad {
    ...
    self.bet_bao1_View = [LotteryBetView new];
    self.bet_bao6_View = [LotteryBetView new];
    self.bet_red_View = [LotteryBetView new];
    self.bet_black_View = [LotteryBetView new];
    ...
    [stackView1 addArrangedSubview:self.bet_bao1_View];
    [stackView1 addArrangedSubview:self.bet_red_View];
    ...
    [stackView2 addArrangedSubview:self.bet_small_View];
    [stackView2 addArrangedSubview:self.bet_double_View];
    [stackView2 addArrangedSubview:self.bet_bao6_View];
    [stackView2 addArrangedSubview:self.bet_black_View];
    ...
    NSArray<UIView*> *betViews = @[self.bet_big_View,self.bet_small_View,self.bet_single_View,self.bet_double_View,self.bet_bao1_View,self.bet_bao6_View,self.bet_red_View,self.bet_black_View];
    ...
    [[YBNetworking sharedManager] postNetworkWithUrl:getOpenHistoryUrl withBaseDomian:YES andParameter:@{@"page":minnum(openPage)} data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if(code == 0 && [info isKindOfClass:[NSDictionary class]])
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                strongSelf.openResutLab.text = minstr(strongSelf->last_open_result);
                strongSelf.openResutLab.hidden = NO;
                [strongSelf resultColor:strongSelf->last_open_result];
            });
        }
        else{
            strongSelf.openResutLab.text = minstr(strongSelf->last_open_result);
...
//        [self.openResultCollection reloadData];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if(strongSelf.selectedToolBtn.tag != 1002){
...
- (void)tableAndChipsChangedWithWinways:(NSArray *)winways{
    ...
    for (NSString *winName in winways) {
        UIImageView *imgV;
        if ([winName isEqualToString:@"点数_0"]) {
        ...
        }else if ([winName isEqualToString:@"点数_大"]){
            ...
            imgV = [self.bet_black_View viewWithTag:300];
            [winViews addObject:self.bet_black_View];
    ...
    [UIView animateWithDuration:0.8 delay:2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (strongSelf->winUsersArray.count>0) {
            ...
            for (LotteryProfit *betWinModel in strongSelf->winUsersArray) {
                ...
                if ([winWaysStr isEqualToString:@"点数_0"]||[winWaysStr isEqualToString:@"颜色_红色"]||[winWaysStr isEqualToString:@"点数_1-12"]||[winWaysStr isEqualToString:@"颜色_黑色"]||[winWaysStr isEqualToString:@"点数_13-24"]||[winWaysStr isEqualToString:@"点数_大"]||[winWaysStr isEqualToString:@"点数_25-36"]||[winWaysStr isEqualToString:@"点数_小"]) {
            ...
            if (dicWinWays.count>0) {
                ...
                for (int i = 0; i<winWaysStrs.count; i++) {
                    ...
                    if ([winSubWay isEqualToString:@"点数_0"]) {
                        tagChipsub = 20000;
                    }else if([winSubWay isEqualToString:@"颜色_红色"]){
                        tagChipsub = 20001;
                    }else if([winSubWay isEqualToString:@"点数_1-12"]){
                        tagChipsub = 20002;
                    }else if([winSubWay isEqualToString:@"颜色_黑色"]){
                        tagChipsub = 20003;
                    }else if([winSubWay isEqualToString:@"点数_13-24"]){
                        tagChipsub = 20004;
                    }else if([winSubWay isEqualToString:@"点数_大"]){
                        tagChipsub = 20005;
                    }else if([winSubWay isEqualToString:@"点数_25-36"]){
                        tagChipsub = 20006;
                    }else if([winSubWay isEqualToString:@"点数_小"]){
                        tagChipsub = 20007;
                    }
...
-(void)closeAndReadyToBegain
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (strongSelf.bet_black_View) [arraybgs addObject:strongSelf.bet_black_View];
...
//            }
            ...
            }else if (viewSub == strongSelf.bet_red_View){
                for (NSDictionary *dc in strongSelf->_titleSts) {
...
-(void)betNotification:(NSNotification*)notification
{
    ...
    if (betModel.msg.issue == [self.curIssue longLongValue] && curLotteryType == betModel.msg.lotteryType) {
        ...
        for (int i = 0; i<arrWays.count; i++) {
            ...
            if ([subWay isEqualToString:@"点数_0"]) {
                keyContinue = @"1";
                ...
                keyContinue = @"5";
                [self animationFrom:fromView toView:self.bet_bao1_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20004 moneyNum:subMoney] uid:betModel.msg.uid];
            }else if ([subWay isEqualToString:@"点数_大"]){
                keyContinue = @"6";
                [self animationFrom:fromView toView:self.bet_bao6_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20005 moneyNum:subMoney] uid:betModel.msg.uid];
            }else if ([subWay isEqualToString:@"点数_25-36"]){
                keyContinue = @"7";
                [self animationFrom:fromView toView:self.bet_red_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20006 moneyNum:subMoney] uid:betModel.msg.uid];
            }else if ([subWay isEqualToString:@"点数_小"]){
                keyContinue = @"8";
                [self animationFrom:fromView toView:self.bet_black_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20007 moneyNum:subMoney] uid:betModel.msg.uid];
            }
...
//    animationShaigu.hidden = YES;
    ...
    resultPopView.center = CGPointMake(SCREEN_WIDTH/2, heightView/2-100-ShowDiff);
    [resultPopView mas_makeConstraints:^(MASConstraintMaker *make) {
        // 这里假设你想要设置lotterySubBetV的左上角坐标（x，y）相对于self.contentView的左上角
        make.left.equalTo(self.contentView.mas_left).offset(resultPopView.x); // 设置x坐标偏移
        make.bottom.mas_equalTo((160)/2-ShowDiff);
        // 如果需要设置lotterySubBetV的宽度和高度，可以使用下面的代码
...
- (void)clickHistoryBetAction:(BetListDataModel *)model {
    ...
    [model.detail.way enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *keyContinue = @"";
        if ([obj isEqualToString:@"点数_0"]) {
            keyContinue = @"1";
        }else if ([obj isEqualToString:@"颜色_红色"]){
            keyContinue = @"2";
        }else if ([obj isEqualToString:@"点数_1-12"]){
            keyContinue = @"3";
        }else if ([obj isEqualToString:@"颜色_黑色"]){
            keyContinue = @"4";
        }else if ([obj isEqualToString:@"点数_13-24"]){
            keyContinue = @"5";
        }else if ([obj isEqualToString:@"点数_大"]){
            keyContinue = @"6";
        }else if ([obj isEqualToString:@"点数_25-36"]){
            keyContinue = @"7";
        }else if ([obj isEqualToString:@"点数_小"]){
            keyContinue = @"8";
        }
...
//        if (name&&[name isEqualToString:@""]) {
            for (int x = 0; x<options.count; x++) {
                ...
                if (title&&[title isEqualToString:@"点数_0"]) {
                    ...
                    UILabel *betAllLabel = [self.bet_bao1_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_bank_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_bao1_View viewWithTag:1010];
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = titleSt;
                    valueLabel.text = [@"X" stringByAppendingString:value];
                ...
                }else if(title&&[title isEqualToString:@"点数_大"]){
                    ...
                    UILabel *betAllLabel = [self.bet_bao6_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_bank_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_bao6_View viewWithTag:1010];
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = titleSt;
                    valueLabel.text = [@"X" stringByAppendingString:value];
                    ...
                    toView = self.bet_black_View;
                    tagChipN = 20007;
                    UILabel *titleLabel = [self.bet_black_View viewWithTag:200];
                    UILabel *valueLabel = [self.bet_black_View viewWithTag:201];
                    UILabel *betAllLabel = [self.bet_black_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_bank_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_black_View viewWithTag:1010];
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = titleSt;
                    valueLabel.text = [@"X" stringByAppendingString:value];
...
//    });
    ...
    UINib *copennib=[UINib nibWithNibName:kLotteryOpenViewCell_ZP bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
    [self.openResultList registerNib: copennib forCellWithReuseIdentifier:kLotteryOpenViewCell_ZP];
...
-(void)betWithView:(UIView *)tapView chipNum:(NSInteger)chipNum
{
    ...
    if ([tapView isEqual:self.bet_big_View]) {
        [self doBetType:@"点数_0" chipNum:chipNum toView:self.bet_big_View];
    }else if ([tapView isEqual:self.bet_small_View]){
        [self doBetType:@"颜色_红色" chipNum:chipNum toView:self.bet_small_View];
    }else if ([tapView isEqual:self.bet_single_View]){
        [self doBetType:@"点数_1-12" chipNum:chipNum toView:self.bet_single_View];
    }else if ([tapView isEqual:self.bet_double_View]){
        [self doBetType:@"颜色_黑色"chipNum:chipNum toView:self.bet_double_View];
    }else if ([tapView isEqual:self.bet_bao1_View]){
        [self doBetType:@"点数_13-24" chipNum:chipNum toView:self.bet_bao1_View];
    }else if ([tapView isEqual:self.bet_bao6_View]){
        [self doBetType:@"点数_大" chipNum:chipNum toView:self.bet_bao6_View];
    }else if ([tapView isEqual:self.bet_red_View]){
        [self doBetType:@"点数_25-36" chipNum:chipNum toView:self.bet_red_View];
    }else if ([tapView isEqual:self.bet_black_View]){
        [self doBetType:@"点数_小" chipNum:chipNum toView:self.bet_black_View];
    }
}
...
-(void)doBetType:(NSString*)detailType chipNum:(NSInteger)chipNum toView:(UIView*)toView {
    ...
    if ([detailType isEqualToString:@"点数_0"]) {
    ...
    }else if ([detailType isEqualToString:@"颜色_红色"]){
    ...
    }else if ([detailType isEqualToString:@"点数_1-12"]){
    ...
    }else if ([detailType isEqualToString:@"颜色_黑色"]){
    ...
    }else if ([detailType isEqualToString:@"点数_13-24"]){
        [self animationFrom:fromView toView:self.bet_bao1_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20004 moneyNum:chipNumStr] uid:0];
    }else if ([detailType isEqualToString:@"点数_大"]){
        [self animationFrom:fromView toView:self.bet_bao6_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20005 moneyNum:chipNumStr] uid:0];
    }else if ([detailType isEqualToString:@"点数_25-36"]){
        [self animationFrom:fromView toView:self.bet_red_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20006 moneyNum:chipNumStr] uid:0];
    }else if ([detailType isEqualToString:@"点数_小"]){
        [self animationFrom:fromView toView:self.bet_black_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20007 moneyNum:chipNumStr] uid:0];
    }
...
//            [MBProgressHUD showError:msg];
            // 清空信息
            NSArray<UIView*> *betViews = @[strongSelf.bet_big_View,strongSelf.bet_small_View,strongSelf.bet_single_View,strongSelf.bet_double_View,strongSelf.bet_bao1_View,strongSelf.bet_bao6_View,strongSelf.bet_red_View,strongSelf.bet_black_View];
            for (int i = 0; i<betViews.count; i++) {
...
//                            [subV sd_setImageWithURL:[NSURL URLWithString:urlPhoto] placeholder:[ImageBundle imagewithBundleName:@"top_avatar"] options:YYWebImageOptionAllowBackgroundTask progress:nil transform:nil completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
//
//                            }];
