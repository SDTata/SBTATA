...
#define heightView LotteryWindowNewHeighZJH

@interface LotteryBetViewController_ZJH(){
    BOOL isShow;
...
@implementation LotteryBetViewController_ZJH
...
- (void)viewDidLoad {
    ...
    self.v_poker_area = ({
        ...
        self.v_player1_Area = [LotteryBetView_ZJH new];
        self.v_player1_Area.type = ZJHLotteryBetViewTypeLeft;
        [view addSubview:self.v_player1_Area];
        
        self.v_player2_Area = [LotteryBetView_ZJH new];
        self.v_player2_Area.type = ZJHLotteryBetViewTypeCenter;
        [view addSubview:self.v_player2_Area];
        
        self.v_player3_Area = [LotteryBetView_ZJH new];
        self.v_player3_Area.type = ZJHLotteryBetViewTypeRight;
        [view addSubview:self.v_player3_Area];
        
        NSArray *subViews = @[self.v_player1_Area, self.v_player2_Area, self.v_player3_Area];
        [subViews mas_distributeViewsAlongAxis:MASAxisTypeHorizontal withFixedSpacing:5 leadSpacing:0 tailSpacing:0];
    ...
    self.bet_player1_View = [LotteryBetView new];
    self.bet_player2_View = [LotteryBetView new];
    self.bet_player3_View = [LotteryBetView new];
    ...
    [stackView1 addArrangedSubview:self.v_poker_area];
    ...
    [stackView2 addArrangedSubview:self.bet_player1_View];
    [stackView2 addArrangedSubview:self.bet_player2_View];
    [stackView2 addArrangedSubview:self.bet_player3_View];
    ...
    NSArray<UIView*> *betViews = @[self.bet_player1_View,self.bet_player2_View,self.bet_player3_View];
...
- (void)getOpenResultInfo{
    ...
    [[YBNetworking sharedManager] postNetworkWithUrl:getOpenHistoryUrl withBaseDomian:YES andParameter:@{@"page":minnum(openPage)} data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if(code == 0 && [info isKindOfClass:[NSDictionary class]])
        {
            ...
            NSArray *dataArrays = [infoDict objectForKey:@"list"];
...
- (void)refreshLastOpen:(NSNotification *)notification {
    ...
    if(curLotteryType>0 && [lotteryType isEqualToString:[NSString stringWithFormat:@"%ld",curLotteryType]]){
        ...
        //关闭骰子动画显示结果
//        ---- 开牌 ----
        NSDictionary *bjlNum = notification.userInfo[@"zjh"];
...
- (void)tableAndChipsChangedWithWinways:(NSArray *)winways{
    ...
    for (NSString *winName in winways) {
        ...
        if ([winName isEqualToString:@"猜胜负_玩家一"]) {
            ...
            self.v_player1_Area.selected = YES;
            self.bet_player1_View.selected = YES;
            ...
            self.v_player2_Area.selected = YES;
            self.bet_player2_View.selected = YES;
            ...
            self.v_player3_Area.selected = YES;
            self.bet_player3_View.selected = YES;
        }
    }
    ...
    [UIView animateWithDuration:0.8 delay:2 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (strongSelf->winUsersArray.count>0) {
            ...
            for (LotteryProfit *betWinModel in strongSelf->winUsersArray) {
                ...
                //0-0
                if ([winWaysStr isEqualToString:@"猜胜负_玩家一"]||[winWaysStr isEqualToString:@"猜胜负_玩家二"]||[winWaysStr isEqualToString:@"猜胜负_玩家三"]) {
            ...
            if (dicWinWays.count>0) {
                ...
                for (int i = 0; i<winWaysStrs.count; i++) {
                    ...
                    //0-0
                    if ([winSubWay isEqualToString:@"猜胜负_玩家一"]) {
                        tagChipsub = 20000;
                    }else if([winSubWay isEqualToString:@"猜胜负_玩家二"]){
                        tagChipsub = 20002;
                    }else if([winSubWay isEqualToString:@"猜胜负_玩家三"]){
                        tagChipsub = 20004;
                    }
            ...
            if (seconds<=0) {
                ...
                dispatch_main_async_safe(^{
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        ...
                        for (int i = 0; i < 3; i++) {
                            for (int j = 0; j < 3; j++) {
                                CGFloat pWidth = strongSelf.v_player1_Area.areaView.height * 37 / 49.0;
                                CGFloat pHeight = strongSelf.v_player1_Area.areaView.height;
...
//                        imgV.image = [ImageBundle imagewithBundleName:@"bet_bolder"];
                    ...
                    self.v_player1_Area.selected = NO;
                    self.v_player2_Area.selected = NO;
                    self.v_player3_Area.selected = NO;
    ...
    if (betModel.msg.issue == [self.curIssue longLongValue] && curLotteryType == betModel.msg.lotteryType) {
        ...
        for (int i = 0; i<arrWays.count; i++) {
            ...
            if ([subWay isEqualToString:@"猜胜负_玩家一"]) {
                keyContinue = @"1";
                [self animationFrom:fromView toView:self.bet_player1_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20000 moneyNum:subMoney] uid:betModel.msg.uid];
            }else if ([subWay isEqualToString:@"猜胜负_玩家二"]){
                keyContinue = @"2";
                [self animationFrom:fromView toView:self.bet_player2_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20002 moneyNum:subMoney] uid:betModel.msg.uid];
            }else if ([subWay isEqualToString:@"猜胜负_玩家三"]){
                keyContinue = @"3";
                [self animationFrom:fromView toView:self.bet_player3_View betTotalMoney:subMoney chipView:[self getChipImgViewWithTag:20004 moneyNum:subMoney] uid:betModel.msg.uid];
            }
        ...
        for (UIView *item in self.v_poker_area.subviews) {
            if ([item isKindOfClass:[PockerView class]]) {
                [UIView animateWithDuration:0.2 animations:^{
                    CGFloat pWidth = self.v_player1_Area.areaView.height * 37 / 49.0;
                    CGFloat pHeight = self.v_player1_Area.areaView.height;
...
- (void)showAddPockerAnimation:(PockerView *)pocker{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (row == 0) {
            //Player1
            CGRect covertFrame = [self.v_player1_Area.areaView convertRect:self.v_player1_Area.areaView.bounds toView:self.v_poker_area];
            ...
            //player2
            CGRect covertFrame = [self.v_player2_Area.areaView convertRect:self.v_player2_Area.areaView.bounds toView:self.v_poker_area];
            CGRect targetFrame = CGRectMake(covertFrame.origin.x + offset, covertFrame.origin.y + 1, pocker.width, pocker.height);
            [UIView animateWithDuration:0.3 animations:^{
                pocker.frame = targetFrame;
                [self.contentView layoutIfNeeded];
            ...
            //player3
            CGRect covertFrame = [self.v_player3_Area.areaView convertRect:self.v_player3_Area.areaView.bounds toView:self.v_poker_area];
            CGRect targetFrame = CGRectMake(covertFrame.origin.x + offset, covertFrame.origin.y + 1, pocker.width, pocker.height);
            [UIView animateWithDuration:0.3 animations:^{
                pocker.frame = targetFrame;
                [self.contentView layoutIfNeeded];
...
- (void)clickHistoryBetAction:(BetListDataModel *)model {
    ...
    [model.detail.way enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *keyContinue = @"";
        if ([obj isEqualToString:@"猜胜负_玩家一"]) {
            keyContinue = @"1";
        }else if ([obj isEqualToString:@"猜胜负_玩家二"]){
            keyContinue = @"2";
        }else if ([obj isEqualToString:@"猜胜负_玩家三"]){
            keyContinue = @"3";
        }
        ...
        if (name&&[name isEqualToString:@"炸金花"]) {
            for (int x = 0; x<options.count; x++) {
                ...
                if (title&&[title isEqualToString:@"猜胜负_玩家一"]) {
                    ...
                    tagChipN = 20000;
                    UILabel *titleLabel = [self.bet_player1_View viewWithTag:200];
                    UILabel *valueLabel = [self.bet_player1_View viewWithTag:201];
                    UILabel *betAllLabel = [self.bet_player1_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_player_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_player1_View viewWithTag:1010];
                    ...
                    titleLabel.font = vkFontMedium(16);
                    valueLabel.font = vkFontMedium(12);
                    ...
                    tagChipN = 20002;
                    UILabel *titleLabel = [self.bet_player2_View viewWithTag:200];
                    UILabel *valueLabel = [self.bet_player2_View viewWithTag:201];
                    UILabel *betAllLabel = [self.bet_player2_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_player_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_player2_View viewWithTag:1010];
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = titleSt;
                    valueLabel.text = [@"x" stringByAppendingString:value];
                    ...
                    tagChipN = 20004;
                    UILabel *titleLabel = [self.bet_player3_View viewWithTag:200];
                    UILabel *valueLabel = [self.bet_player3_View viewWithTag:201];
                    UILabel *betAllLabel = [self.bet_player3_View viewWithTag:101];
                    //UILabel *betMineLabel = [self.bet_equal_View viewWithTag:101];
                    BetBubbleView *bubble = [self.bet_player3_View viewWithTag:1010];
                    betAllLabel.text = minnum(betall);
                    [bubble setBetCount:titleSt];
                    titleLabel.text = titleSt;
                    valueLabel.text = [@"X" stringByAppendingString:value];
...
//    });
    ...
    UINib *copennib=[UINib nibWithNibName:kLotteryOpenViewCell_ZJH bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
    [self.openResultList registerNib: copennib forCellWithReuseIdentifier:kLotteryOpenViewCell_ZJH];
...
-(void)doBetType:(NSString*)detailType chipNum:(NSInteger)chipNum toView:(UIView*)toView {
    ...
    if ([detailType isEqualToString:@"猜胜负_玩家一"]) {
        [self animationFrom:fromView toView:self.bet_player1_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20000 moneyNum:chipNumStr] uid:0];
    }else if ([detailType isEqualToString:@"猜胜负_玩家二"]){
        [self animationFrom:fromView toView:self.bet_player2_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20002 moneyNum:chipNumStr] uid:0];
    }else if ([detailType isEqualToString:@"猜胜负_玩家三"]){
        [self animationFrom:fromView toView:self.bet_player3_View betTotalMoney:chipNumStr chipView:[self getChipImgViewWithTag:20004 moneyNum:chipNumStr] uid:0];
    }
...
//            [MBProgressHUD showError:msg];
            // 清空信息
            NSArray<UIView*> *betViews = @[strongSelf.bet_player1_View,strongSelf.bet_player2_View,strongSelf.bet_player3_View];
            for (int i = 0; i<betViews.count; i++) {
