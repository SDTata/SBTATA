...
#import "LotteryCenterBetViewController_BJL.h"
...
@interface LotteryCenterBetViewController_BJL()<socketDelegate>{
...
@implementation LotteryCenterBetViewController_BJL
...
- (void)viewDidLoad {
    ...
    chartSubV = [ChartView instanceChatViewWithType:28];
    ...
    if(curLotteryType>0 && [lotteryType isEqualToString:[NSString stringWithFormat:@"%ld",curLotteryType]]){
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (strongSelf->chartSubV) {
                [strongSelf->chartSubV updateChartData:[whoWinStr stringByReplacingOccurrencesOfString:@"百家乐_" withString:@""]];
            }
        });
...
- (void)tableAndChipsChangedWithWinways:(NSArray *)winways{
    ...
    for (NSString *winName in winways) {
        UIImageView *imgV;
        //0-0
        if ([winName rangeOfString:@"百家乐_闲胜"].location!=NSNotFound) {
            ...
            if (seconds<=0) {
                ...
                dispatch_main_async_safe((^{
                    ...
                    for (UIView *viewSub in arraybgs) {
                        ...
                        imgV.image = [ImageBundle imagewithBundleName:@"bjl_betbg"];
...
-(void)betNotification:(NSNotification*)notification
{
    ...
    if (betModel.msg.issue == [curIssue longLongValue] && curLotteryType == betModel.msg.lotteryType) {
        ...
        for (int i = 0; i<arrWays.count; i++) {
            ...
            //0-0
            if ([subWay rangeOfString:@"百家乐_闲胜"].location!=NSNotFound) {
                keyContinue = @"1";
            ...
            }else if ([subWay rangeOfString:@"百家乐_闲对"].location!=NSNotFound){
                keyContinue = @"2";
            ...
            }else if ([subWay rangeOfString:@"百家乐_和"].location!=NSNotFound){
                keyContinue = @"3";
            ...
            }else if ([subWay rangeOfString:@"百家乐_超级6点"].location!=NSNotFound){
                keyContinue = @"4";
            ...
            }else if ([subWay rangeOfString:@"百家乐_庄胜"].location!=NSNotFound){
                keyContinue = @"5";
            ...
            }else if ([subWay rangeOfString:@"百家乐_庄对"].location!=NSNotFound){
                keyContinue = @"6";
...
//    });
    ...
    UINib *copennibLH=[UINib nibWithNibName:kLotteryOpenViewCell_BJL bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
    //[self.openResultCollection registerNib: copennibLH forCellWithReuseIdentifier:kLotteryOpenViewCell_LH];
    [self.openResultList registerNib: copennibLH forCellWithReuseIdentifier:kLotteryOpenViewCell_BJL];
...
-(void)doBetType:(NSString*)detailType chipNum:(NSInteger)chipNum toView:(UIView*)toView{
    ...
    if ([detailType rangeOfString:@"百家乐_闲胜"].location!=NSNotFound) {
    ...
    }else if ([detailType rangeOfString:@"百家乐_闲对"].location!=NSNotFound){
    ...
    }else if ([detailType rangeOfString:@"百家乐_和"].location!=NSNotFound){
    ...
    }else if ([detailType rangeOfString:@"百家乐_超级6点"].location!=NSNotFound){
    ...
    }else if ([detailType rangeOfString:@"百家乐_庄胜"].location!=NSNotFound){
    ...
    }else if ([detailType rangeOfString:@"百家乐_庄对"].location!=NSNotFound){
...
//        [MBProgressHUD showError:YZMsg(@"LobbyBet_selecte_Warning")];
    ...
    WeakSelf
    NSString *betUrl = [NSString stringWithFormat:@"Lottery.Betting&uid=%@&token=%@&lottery_type=%@&money=%@&way=%@&serTime=%@&issue=%@&optionName=%@&liveuid=%@&betid=%@",[Config getOwnID],[Config getOwnToken],lottery_type,money,way,minnum([[NSDate date] timeIntervalSince1970]),issue,@"百家乐",@"0",betIdStr];//User.getPlats
