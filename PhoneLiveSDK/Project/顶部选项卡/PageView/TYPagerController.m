...
NS_INLINE NSRange visibleRangWithOffset(CGFloat offset,CGFloat width, NSInteger maxIndex)
{
    ...
    NSInteger endIndex = ceil((offset*1.0 + width)/width);
...
//    CGFloat contentTopEdging = _contentTopEdging + [self statusBarHeight] + statusbarHeight;
    ...
    _contentView.frame = CGRectMake(0, 0, _window_width, self.view.height);
...
- (void)configurePagerIndex
{
    ...
    if (direction == TYPagerControllerLeft) {
        ...
        index = ceil(offsetX*1.0/width-percentChangeIndex);
    }
...
- (void)removeControllersOutOfVisibleRange:(NSRange)range
{
    ...
    [_visibleControllers enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, UIViewController *viewController, BOOL * stop) {
        ...
        if (!NSLocationInRange(indexOfController, range)) {
            // unvisible
            [strongSelf removeViewController:viewController atIndex:indexOfController];
            [deleteArray addObject:key];
        }else {
            [strongSelf addViewController:viewController atIndex:indexOfController];
        }
    }];
