...
#define _pageBarWidth  _window_width *0.78

#define kCollectionViewBarHieght 44
#define kUnderLineViewHeight 2

#define KPaddingRight 150
...
- (void)viewDidLoad {
    ...
    if (@available(iOS 11.0, *)) {
        self.collectionViewBar.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
    }else{
...
{
    _pagerBarView = [[UIView alloc]initWithFrame:CGRectMake(0,0,CGRectGetWidth(self.view.frame), State_Bar_H + self.contentTopEdging)];
    _pagerBarView.backgroundColor = [UIColor whiteColor];
    //[self.pagerBarView addGradientView:RGB_COLOR(@"#fc466b", 1) endColor:RGB_COLOR(@"#3f5efb", 1)];
    //[self.pagerBarView addGradientView:RGB_COLOR(@"#ffffff", 1) endColor:RGB_COLOR(@"#ffffff", 1)];
...
- (void)addCollectionViewBar
{
    ...
    UICollectionView *collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0,0 + statusbarHeight, _window_width-KPaddingRight, self.contentTopEdging) collectionViewLayout:layout];
    //[collectionView addGradientView:RGB_COLOR(@"#fc466b", 1) endColor:RGB_COLOR(@"#3f5efb", 1)];
    //[collectionView addGradientView:RGB_COLOR(@"#ffffff", 1) endColor:RGB_COLOR(@"#ffffff", 1)];
    ...
    if (_cellContainXib) {
        UINib *nib = [UINib nibWithNibName:_cellId bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
...
- (void)layoutTabPagerView
{
    ...
    ((UICollectionViewFlowLayout *)_collectionViewBar.collectionViewLayout).minimumInteritemSpacing = 0;
    ((UICollectionViewFlowLayout *)self.collectionViewBar.collectionViewLayout).sectionInset = UIEdgeInsetsMake(0,0,0,0);
    _pagerBarView.frame = CGRectMake(0,0,CGRectGetWidth(self.view.frame),44+statusbarHeight);
    _collectionViewBar.frame = CGRectMake(0,statusbarHeight/2+20,_window_width-KPaddingRight,44);
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:_animateDuration animations:^{
            ...
            strongSelf.progressView.frame = CGRectMake(progressX + width/2-18,progressY+5, 36, strongSelf.progressHeight+1);
        }];
    }else {
        _progressView.frame = CGRectMake(progressX + width/2-18,progressY+5, 36, _progressHeight+1);
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MXTabCell *cell = (MXTabCell*)[collectionView dequeueReusableCellWithReuseIdentifier:_cellId forIndexPath:indexPath];
    
//    if (self.curIndex == indexPath.row) {
//        cell.titleLabel.transform = CGAffineTransformIdentity;
//    }else{
//        cell.titleLabel.transform = CGAffineTransformMakeScale(self.selectFontScale, self.selectFontScale);
//    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_cellWidth > 0) {
        return CGSizeMake(_pageBarWidth/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
    }else if(_tabDataSourceFlags.titleForIndex){
//        if ([self.dataSource numberOfControllersInPagerController]<=2) {
//            return CGSizeMake(_pageBarWidth/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
//        }
        CGSize size = [self boundingSizeWithString:title font:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake(1000, 50)];
//
        return CGSizeMake(size.width + (indexPath.row>1? 15:15),CGRectGetHeight(_collectionViewBar.frame) - 10);
//        return CGSizeMake(_pageBarWidth/5,CGRectGetHeight(_collectionViewBar.frame) - 10);
    }
    return CGSizeZero;
}
