...
- (void)creatUI{
    ...
    titleL.text = YZMsg(@"addPopTypeView_addAccount");
    ...
    label.text = YZMsg(@"addPopTypeView_AddAccountType");
    ...
    jiantouImg = [[UIImageView alloc]initWithImage:[ImageBundle imagewithBundleName:@"profit_right"]];
    ...
    selectTypeLabel.text = YZMsg(@"addPopTypeView_alipay");
...
//        make.top.equalTo(text3.mas_bottom).offset(5);
    ...
    [sureBtn setTitle:YZMsg(@"publictool_sure") forState:0];
...
- (void)updateConstraintsWith:(int)type{
    if (type == 1) {
        text1.placeholder = YZMsg(@"addPopTypeView_input_aplipayAccount");
        text1.keyboardType = UIKeyboardTypeDefault;
        text2.placeholder = YZMsg(@"addPopTypeView_input_aplipayName");
    ...
    if (type == 2) {
        text1.placeholder = YZMsg(@"addPopTypeView_input_WechatAccount");
        text1.keyboardType = UIKeyboardTypeDefault;
        text2.hidden = YES;
        text3.hidden = YES;
        [lineView2 mas_remakeConstraints:^(MASConstraintMaker *make) {
    ...
    if (type == 3) {
        ...
        text1.placeholder = YZMsg(@"addPopTypeView_input_bankName");
        text1.keyboardType = UIKeyboardTypeDefault;
        text2.placeholder = YZMsg(@"addPopTypeView_input_bankNum");
        text2.keyboardType = UIKeyboardTypeNumberPad;
        text3.placeholder = YZMsg(@"addPopTypeView_input_bankUserName");
...
- (void)selectTypeBtnClick{
    if (!twoBtnView) {
        ...
        if (showType == 1) {
            [btn1 setTitle:YZMsg(@"addPopTypeView_Wechat") forState:0];
            [btn2 setTitle:YZMsg(@"addPopTypeView_banKCard") forState:0];
        }
        if (showType == 2) {
            [btn1 setTitle:YZMsg(@"addPopTypeView_alipay") forState:0];
            [btn2 setTitle:YZMsg(@"addPopTypeView_banKCard") forState:0];
        }
        if (showType == 3) {
            [btn1 setTitle:YZMsg(@"addPopTypeView_alipay") forState:0];
            [btn2 setTitle:YZMsg(@"addPopTypeView_Wechat") forState:0];
        }
...
- (void)changeType:(UIButton *)sender{
    if ([sender.titleLabel.text isEqual:YZMsg(@"addPopTypeView_alipay")]) {
        selectTypeLabel.text = YZMsg(@"addPopTypeView_alipay");
        showType = 1;
    }
    if ([sender.titleLabel.text isEqual:YZMsg(@"addPopTypeView_Wechat")]) {
        selectTypeLabel.text = YZMsg(@"addPopTypeView_Wechat");
        showType = 2;
    }
    if ([sender.titleLabel.text isEqual:YZMsg(@"addPopTypeView_banKCard")]) {
        selectTypeLabel.text = YZMsg(@"addPopTypeView_banKCard");
        showType = 3;
    }
    [twoBtnView removeFromSuperview];
    twoBtnView = nil;
...
- (void)sureBtnClick{
    ...
    if (showType == 1) {
        if (text1.text == nil || text1.text == NULL || text1.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_aplipayAccount")];
            return;
        }
        if (text2.text == nil || text2.text == NULL || text2.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_aplipayName")];
            return;
        }
    ...
    if (showType == 2) {
        if (text1.text == nil || text1.text == NULL || text1.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_WechatAccount")];
            return;
        }
        subdic = @{@"type":@(showType),
                   @"account":text1.text,
    ...
    if (showType == 3) {
        if (text1.text == nil || text1.text == NULL || text1.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_bankName")];
            return;
        }
        if (text2.text == nil || text2.text == NULL || text2.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_bankNum")];
            return;
        }
        if (text3.text == nil || text3.text == NULL || text3.text.length == 0) {
            [MBProgressHUD showError:YZMsg(@"addPopTypeView_input_bankUserName")];
            return;
        }
        subdic = @{@"type":@(showType),
                   @"account":text2.text,
    ...
    [[YBNetworking sharedManager] postNetworkWithUrl:@"User.SetUserAccount" withBaseDomian:YES andParameter:subdic data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if (code == 0) {
            ...
            strongSelf->text1.text = @"";
            strongSelf->text2.text = @"";
            strongSelf->text3.text = @"";
            strongSelf.block();
        }else{
            [MBProgressHUD showError:msg];
        }
