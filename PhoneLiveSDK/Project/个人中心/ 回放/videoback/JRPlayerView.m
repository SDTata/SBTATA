...
- (void)setImage:(NSString *)imageUrl {
	...
	[self.appearBtn setImage:[ImageBundle imagewithBundleName:@"play.png"] forState:UIControlStateNormal];
			...
			if (self.smallControlView.isShow) {
				...
				[self.smallControlView openSlidView:^{
					[UIView animateWithDuration:0.25 animations:^{
						...
						strongSelf.smallControlView.alpha = 0.0;
					} completion:^(BOOL finished) {
                        STRONGSELF
                        if (strongSelf == nil) {
                            return;
                        }
						strongSelf.smallAppear = NO;
					}];
				}];
			} else {
                WeakSelf
