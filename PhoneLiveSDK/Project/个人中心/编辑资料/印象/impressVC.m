...
-(void)navtion{
    ...
    label.text = YZMsg(@"otherUserMsgVC_impressed");
    ...
    if (_isAdd) {
        ...
        [saveBtn setTitle:YZMsg(@"public_Save") forState:0];
...
- (void)viewDidLoad {
    ...
    mineL.adjustsFontSizeToFitWidth = YES;
    mineL.minimumScaleFactor = 0.5;
    mineL.textColor = RGB(99, 100, 101);
    if (_isAdd) {
        mineL.text = YZMsg(@"impressVC_anchorImpressed_choise");
        selectMutableArray = [NSMutableArray array];
    }else{
        mineL.text = YZMsg(@"impressVC_MyImpressed");
    }
...
//    impressCollection.scrollEnabled = NO;
    ...
    [impressCollection registerNib:[UINib nibWithNibName:@"impressCell" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]] forCellWithReuseIdentifier:@"impressCELL"];
...
WeakSelf
    [[YBNetworking sharedManager] postNetworkWithUrl:name withBaseDomian:YES andParameter:dic data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
        if (code == 0) {
            if (strongSelf.isAdd) {
                for (NSDictionary *diccc in info) {
                    if ([minstr([diccc valueForKey:@"ifcheck"]) isEqual:@"1"]) {
                        [strongSelf->selectMutableArray addObject:diccc];
                    }
                }
            }else{
                if ([info count] == 0) {
                    strongSelf->mineL.text = YZMsg(@"impressVC_NoImpressed");
                    strongSelf->mineL.font = [UIFont systemFontOfSize:13];
                }
            }
            NSArray *list = info;
            ...
            strongSelf->infoArray = muArr2;

        
            [strongSelf->impressCollection reloadData];
        }
    ...
    if (_isAdd) {
        ...
        if (isCons) {
            ...
            if (selectMutableArray.count >= 3) {
                [MBProgressHUD showError:YZMsg(@"impressVC_Choise_limitNum")];
                return;
            }
...
- (void)doSave{
    if (selectMutableArray.count == 0) {
        [MBProgressHUD showError:YZMsg(@"impressVC_Choise_min_tip")];
        return;
    }
    NSString *str = @"";
    for (NSDictionary *dic in selectMutableArray) {
    ...
    WeakSelf
    [[YBNetworking sharedManager] postNetworkWithUrl:@"User.setUserLabel" withBaseDomian:YES andParameter:parameter data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
