...
@interface AddPpViewController ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate,UITextViewDelegate,AddUSDTInfoCell>
{
...
@implementation AddPpViewController

-(void)navtion{
    UIView *navtion = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _window_width, 64 + statusbarHeight)];
    navtion.backgroundColor =navigationBGColor;
    UILabel *label = [[UILabel alloc]init];
    label.text = [NSString stringWithFormat:YZMsg(@"AddOther_title%@"),self.titleStr];
...
- (void)loadData {
    ...
    self.infoModel.bankNameStr = self.titleStr;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        ...
        case 1:
        {
            ...
            cell.canEdit = YES;
            cell.textField.placeholder = [NSString stringWithFormat:YZMsg(@"AddOther_Address_title%@"),self.titleStr];
            cell.textField.returnKeyType = UIReturnKeyDone;
...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        [BindCardBtn setTitle: [NSString stringWithFormat:YZMsg(@"AddOther_title%@"),self.titleStr] forState:0];
...
-(void) bindCardClick:(UIButton *)sender{
    NSLog(@"-----保存数据-----");
    NSLog(@"真实姓名：%@", self.infoModel.nameStr);
    ...
    if (self.infoModel.cardNumberStr == nil || self.infoModel.cardNumberStr == NULL || self.infoModel.cardNumberStr.length == 0) {
        [MBProgressHUD showError:[NSString stringWithFormat:YZMsg(@"AddOther_Address_Tips%@"),self.titleStr]];
        return;
    }
  
    NSDictionary *subdic = @{@"type":@(self.typeNum.intValue),
              @"account":self.infoModel.cardNumberStr,//地址
              @"name":self.infoModel.nameStr,//名称
              @"account_bank":self.infoModel.bankNameStr,
...
- (NSArray *)titleArr {
    if (!_titleArr) {
        _titleArr=@[YZMsg(@"AddCard_CardUserName_title"),[NSString stringWithFormat:YZMsg(@"AddOther_Address_title%@"),self.titleStr]];
    }
    return _titleArr;
}
