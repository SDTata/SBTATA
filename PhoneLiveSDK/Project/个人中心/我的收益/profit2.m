...
- (void)viewDidLoad {
   ...
   // input.layer.cornerRadius = 20;
    ...
    input.placeholder = YZMsg(@"profit2_input_withDraw_money");
    ...
    [tixian setTitle:YZMsg(@"public_WithDraw") forState:UIControlStateNormal];
    [tixian setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    tixian.backgroundColor = backColorsss;
...
-(void)tixian{
    if (input.text.length<=0 || [input.text isEqualToString:@"0"]) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:YZMsg(@"profit2_input_money") message:nil delegate:self
                                             cancelButtonTitle:YZMsg(@"publictool_sure") otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    
    NSString *url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=User.userCash&uid=%@&token=%@&money=%@",[Config getOwnID],[Config getOwnToken],input.text];
    WeakSelf
    [YBNetworking getNetWorkWithUrl:url andParameter:nil success:^(int code, NSArray *info, NSString *msg) {
        STRONGSELF
        if (strongSelf==nil) {
            return;
        }
        ...
        if(code == 0)
        {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:info message:nil delegate:self cancelButtonTitle:YZMsg(@"publictool_sure") otherButtonTitles:nil, nil];
            [alert show];
            ...
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:msg message:nil delegate:self cancelButtonTitle:YZMsg(@"publictool_sure") otherButtonTitles:nil, nil];
...
-(void)navtion{
    ...
    labels.text = YZMsg(@"public_WithDraw");
    //label.font = FNOT;
    ...
    [returnBtn setImage:[ImageBundle imagewithBundleName:@"me_jiantou"] forState:UIControlStateNormal];
