...
@interface market ()<WKNavigationDelegate>
{
    WKWebView *wevView;
    NSURL *url;
    //四个下划线
...
-(void)navtion{
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:@[YZMsg(@"market_member"),YZMsg(@"market_liangNum"),YZMsg(@"market_mount")]];
...
//    [EMASCurlProtocol setDebugLogEnabled:YES];
    ...
    if (segment.selectedSegmentIndex == 0) {
        [wevView stopLoading];
        //商城
        NSString *paths = [[DomainManager sharedInstance].domainGetString stringByAppendingFormat:@"/index.php?g=Appapi&m=Vip&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
    ...
    //坐骑
        ...
        [wevView stopLoading];
        NSString *paths = [[DomainManager sharedInstance].domainGetString stringByAppendingFormat:@"/index.php?g=Appapi&m=Car&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        paths = [paths stringByAppendingFormat:@"&l=%@",[YBNetworking currentLanguageServer]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
    ...
    //靓号
        ...
        [wevView stopLoading];
        NSString *paths = [[DomainManager sharedInstance].domainGetString stringByAppendingFormat:@"/index.php?g=Appapi&m=Liang&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        paths = [paths stringByAppendingFormat:@"&l=%@",[YBNetworking currentLanguageServer]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
