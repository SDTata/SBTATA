...
@interface DramaAllListView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation DramaAllListView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DramaAllListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    if (self.videoInfoModels.count > indexPath.row) {
        DramaVideoInfoModel *model = self.videoInfoModels[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(dramaAllListViewDelegateForSelect:)]) {
        DramaVideoInfoModel *model = self.videoInfoModels[indexPath.row];
        [self.delegate dramaAllListViewDelegateForSelect:model.video_id];
    }
    [self close];
}
...
#pragma mark - Action
- (void)close {
    if ([self.delegate respondsToSelector:@selector(dramaAllListViewDelegateForClose)]) {
        [self.delegate dramaAllListViewDelegateForClose];
    }
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[DramaAllListCell self] forCellReuseIdentifier:@"cell"];
