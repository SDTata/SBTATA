...
- (void)creatFollowView{
    ...
    [fCloseBtn setImage:[ImageBundle imagewithBundleName:@"提示关闭"] forState:0];
    ...
    [followBtn setImage:[ImageBundle imagewithBundleName:YZMsg(@"fans_关注")] forState:0];
...
- (void)doFollow{
    ...
    WEAKSELF
    [[YBNetworking sharedManager] postNetworkWithUrl:postUrl  withBaseDomian:YES andParameter:postDic data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        STRONGSELF
...
//            DDLogWarn(@"get the nil message .");
        ...
        if (message.contentType == kJMSGContentTypeEventNotification) {
            if (((JMSGEventContent *)message.content).eventType == kJMSGEventNotificationRemoveGroupMembers
                && ![((JMSGGroup *)strongSelf.conversation.target) isMyselfGroupMember]) {
                //            [strongSelf setupNavigation];
            }
        }
        
        if (strongSelf.conversation.conversationType == kJMSGConversationTypeSingle) {
        } else if (![((JMSGGroup *)strongSelf.conversation.target).gid isEqualToString:((JMSGGroup *)message.target).gid]){
            return;
        }
        
        JCHATChatModel *model = [strongSelf->_allMessageDic objectForKey:message.msgId];
        if (model) {// 说明已经加载，说明可能是同步下来的多媒体消息，下载完成，然后再次收到就去刷新
            model.message = message;
            [strongSelf refreshCellMessageMediaWithChatModel:model];
        }else{
            
            NSString *firstMsgId = [strongSelf->_allmessageIdArr firstObject];
            JCHATChatModel *firstModel = [strongSelf->_allMessageDic objectForKey:firstMsgId];
            ...
            [model setChatModelWith:message conversationType:strongSelf.conversation userModel:strongSelf.userModel];
            if (message.contentType == kJMSGContentTypeImage) {
                [strongSelf->_imgDataArr addObject:model];
            }
            model.photoIndex = [strongSelf->_imgDataArr count] -1;
            ...
            NSMutableArray *messages = strongSelf->_refreshAvatarUsersDic[key];
            if (messages) {
                isHaveCache = YES;
            ...
            [strongSelf->_refreshAvatarUsersDic setObject:messages forKey:key];
...
//            DDLogWarn(@"get the nil message .");
        ...
        if (model) {// 说明已经加载，说明可能是同步下来的多媒体消息，下载完成，然后再次收到就去刷新
            ...
            model = [[JCHATChatModel alloc] init];
            [model setChatModelWith:message conversationType:self.conversation userModel:self.userModel];
            if (message.contentType == kJMSGContentTypeImage) {
...
- (void)photoClick {
    ...
    [lib enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
        ...
        dispatch_main_async_safe(^{
            JCHATPhotoPickerViewController *photoPickerVC = [[JCHATPhotoPickerViewController alloc] initWithNibName:@"JCHATPhotoPickerViewController" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
            photoPickerVC.photoDelegate = self;
            if (strongSelf.presentedViewController == nil) {
                [strongSelf presentViewController:photoPickerVC animated:YES completion:NULL];
            }
...
//            return;
//        }
//        strongSelf->_asrView.frame = CGRectMake(0, 0, _window_width, _window_height);
//        strongSelf.moreViewHeight.constant = ASRHeight-strongSelf.toolBarHeightConstrait.constant;
...
//            return;
//        }
//        strongSelf->_asrView.frame = CGRectMake(0, _window_height, _window_width, _window_height);
//        strongSelf.moreViewHeight.constant = kMoreHeight+ShowDiff;
//        [strongSelf scrollToEnd];
//    }];
//}
...
- (void)dropToolBar {
    ...
    if (_ybNavi.hidden) {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            strongSelf.view.frame = CGRectMake(0, _window_height - _window_height*0.4, _window_width, _window_height*0.4);
            strongSelf->_emojiV.frame = CGRectMake(0, _window_height, _window_width, EmojiHeight);
        }];
        return;
    }
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        strongSelf.toolBarToBottomConstrait.constant = 0+ShowDiff;
        strongSelf->_emojiV.frame = CGRectMake(0, _window_height, _window_width, EmojiHeight+ShowDiff);
        strongSelf.moreViewHeight.constant = 0;
    }];
...
//                return;
//            }
//            strongSelf->_asrMoveUp = YES;
//            strongSelf->_asrView.frame = CGRectMake(0, -keyBoardFrame.size.height, _window_width, _window_height);
//            strongSelf.moreViewHeight.constant = keyBoardFrame.size.height-strongSelf.toolBarHeightConstrait.constant;
//        }];
//    }else{
        [UIView animateWithDuration:animationTime animations:^{
            ...
            strongSelf.moreViewHeight.constant = keyBoardFrame.size.height;
            strongSelf.toolBarToBottomConstrait.constant = 0;
...
- (void)pressMoreBtnClick:(UIButton *)btn {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [strongSelf.toolBarContainer layoutIfNeeded];
        [strongSelf.moreViewContainer layoutIfNeeded];
    }];
...
-(void)pressFaceBtnClick:(UIButton *)btn {
    ...
    if (_ybNavi.hidden) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            strongSelf.view.frame = CGRectMake(0,_window_height*0.2, _window_width, _window_height*0.8 - EmojiHeight);
            strongSelf->_emojiV.frame = CGRectMake(0, _window_height-EmojiHeight, _window_width, EmojiHeight);
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        strongSelf.toolBarToBottomConstrait.constant = EmojiHeight+ShowDiff;
        strongSelf.moreViewHeight.constant = 0;
        strongSelf->_emojiV.frame = CGRectMake(0, _window_height-EmojiHeight-ShowDiff, _window_width, EmojiHeight+ShowDiff);
        [strongSelf.messageTableView layoutIfNeeded];
        [strongSelf.toolBarContainer layoutIfNeeded];
        [strongSelf.moreViewContainer layoutIfNeeded];
    }];
...
- (void)scrollToEnd {
    if ([_allmessageIdArr count] != 0) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:[_allmessageIdArr count]-1 inSection:0];
        if(indexPath.row != _allmessageIdArr.count-1) {
            return;
        }
...
//        DDLogDebug(@"JCHATChatModel is nil%@", messageId);
    ...
    if (model.isTime == YES || model.message.contentType == kJMSGContentTypeEventNotification || model.isErrorMessage) {
        ...
        if (cell == nil) {
            cell = [[[XBundle currentXibBundleWithResourceName:@"JCHATShowTimeCell"] loadNibNamed:@"JCHATShowTimeCell" owner:nil options:nil] lastObject];
    ...
    [fromUser thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) {
        ...
        if (error == nil && [objectId isEqualToString:fromUser.username]) {
            if (data != nil) {
                ...
                [strongSelf refreshVisibleRowsAvatarWithNewMessage:lastMessage avatarDataLength:lenght];
            }
        }
    }];
    //    NSString *key = [NSString stringWithFormat:@"%@_%@",message.fromUser.username,message.fromUser.appKey];
...
//    DDLogDebug(@"refreshVisibleRowsAvatarWithNewMessage::%@",message.serverMessageId);
    ...
    NSArray *indexPaths = [NSArray arrayWithArray:[_messageTableView indexPathsForVisibleRows]];
...
- (void)cancelRecord {
    ...
    [self.voiceRecordHelper cancelledDeleteWithCompletion:^(id xxx) {
...
-(void)checkBlack:(JMSGAbstractContent *)content voice:(NSString *)voicePath {
    [MBProgressHUD showError:YZMsg(@"暂不支持私聊")];
    return;
    ...
    //    }
    //
    //    NSString *url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=User.checkBlack&uid=%@&token=%@&touid=%@",[Config getOwnID],[Config getOwnToken],_userModel.uidStr];
    //    WeakSelf
    ...
    //            for (int i = 0; i < strongSelf->_allmessageIdArr.count; i++) {
    //                NSString *msgid = [strongSelf->_allmessageIdArr objectAtIndex:i];
    //                JCHATChatModel *model = [strongSelf->_allMessageDic objectForKey:msgid];
    ...
    //                }else {
    //                    JMSGMessage *message = nil;
    //                    message = [strongSelf.conversation createMessageWithContent:content];
    //                    JMSGOptionalContent *option;
    //                    option.noSaveNotification = YES;
    //                    [strongSelf->_conversation sendMessage:message optionalContent:option];
    //                    [strongSelf addmessageShowTimeData:message.timestamp];
    //                    JCHATChatModel *model =[[JCHATChatModel alloc] init];
    //                    [model setChatModelWith:message conversationType:strongSelf.conversation userModel:strongSelf.userModel];
    //                    [strongSelf addMessage:model];
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
    ...
    if (changeInHeight != 0.0f) {
        ...
        [UIView animateWithDuration:0.25f
            ...
            [strongSelf setTableViewInsetsWithBottomValue:strongSelf.messageTableView.contentInset.bottom + changeInHeight];
            
            [strongSelf scrollToBottomAnimated:NO];
            
            if (isShrinking) {
                if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
                    strongSelf.previousTextViewContentHeight = MIN(contentH, maxHeight);
                }
                // if shrinking the view, animate text view frame BEFORE input view frame
                [strongSelf.toolBarContainer.toolbar adjustTextViewHeightBy:changeInHeight];
            }
    ...
    if (self.previousTextViewContentHeight == maxHeight) {
        ...
        __weak UITextView *textViewWeak = textView;
        dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
        dispatch_after(popTime,
                       dispatch_get_main_queue(),
                       ^(void) {
            CGPoint bottomOffset = CGPointMake(0.0f, contentH - textViewWeak.bounds.size.height);
            [textViewWeak setContentOffset:bottomOffset animated:YES];
        });
    }
}
...
#pragma mark - 拉黑
-(void)setBlack {
    NSString *url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=User.setBlack&uid=%@&token=%@&touid=%@",[Config getOwnID],[Config getOwnToken],_userModel.uidStr];
    [[YBNetworking sharedManager] postNetworkWithUrl:url  withBaseDomian:NO andParameter:nil data:nil success:^(int code, NSArray *infos, NSString *msg) {
        if (code == 0) {
            NSString *infoDic = [infos firstObject];
