...
- (id)initWithStyle:(UITableViewCellStyle)style
    ...
    if (self) {
        ...
        [_headView setImage:[ImageBundle imagewithBundleName:@"headDefalt.png"]];
        ...
        [self.sendFailView setImage:[ImageBundle imagewithBundleName:@"fail05"]];
...
- (void)reSendMessage {//重发消息
    ...
    UIAlertAction *sureA = [UIAlertAction actionWithTitle:YZMsg(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [strongSelf.sendFailView setHidden:YES];
        [strongSelf.circleView setHidden:NO];
        [strongSelf.circleView startAnimating];
        if (strongSelf.model.message.contentType == kJMSGContentTypeImage) {
            strongSelf.messageContent.alpha = 0.5;
        } else {
            strongSelf.messageContent.alpha = 1;
        }
        if (strongSelf.model.message.contentType == kJMSGContentTypeImage) {
            JMSGImageContent *imgContent = ((JMSGImageContent *)strongSelf.model.message.content);
            ...
            [[JCHATSendMsgManager ins] addMessage:strongSelf.model.message withConversation:strongSelf.conversation];
        } else {
            [strongSelf.conversation sendMessage:strongSelf.model.message];
            [strongSelf layoutAllView];
        }
    }];
        ...
        if (error == nil) {
            if (data != nil) {
                ...
                if (!strongSelf.isPlaying) {
                    ...
                    [[JCHATAudioPlayerHelper shareInstance] setDelegate:(id) strongSelf];
                    strongSelf.isPlaying = YES;
                } else {
                    strongSelf.isPlaying = NO;
                    strongSelf.continuePlayer = NO;
                    [[JCHATAudioPlayerHelper shareInstance] stopAudio];
                    [[JCHATAudioPlayerHelper shareInstance] setDelegate:nil];
                }
                [[JCHATAudioPlayerHelper shareInstance] managerAudioWithData:data toplay:YES];
                [strongSelf changeVoiceImage];
            }
        } else {
//            DDLogDebug(@"Action  voiceData");
            [strongSelf AlertInCurrentViewWithString:@"下载语音数据失败"];
            status = @"获取消息失败。。。";
        }
    }];
    return;
...
- (void)AlertInCurrentViewWithString:(NSString *)string {
    [MBProgressHUD showMessage:string toView:[UIApplication sharedApplication].keyWindow];
}
...
- (void)changeVoiceImage {
    ...
    _messageContent.voiceConent.image = [ImageBundle imagewithBundleName:[NSString stringWithFormat:@"%@%zd", voiceImagePreStr, self.index % 3]];
    if (_isPlaying) {
        self.index++;
...
- (void)didAudioPlayerStopPlay:(AVAudioPlayer *)audioPlayer {
    ...
    self.index = 0;
    if ([_model.message isReceived]) {
        [_messageContent.voiceConent setImage:[ImageBundle imagewithBundleName:@"ReceiverVoiceNodePlaying.png"]];
    } else {
        [_messageContent.voiceConent setImage:[ImageBundle imagewithBundleName:@"SenderVoiceNodePlaying.png"]];
    }
    ...
    [_model.message.fromUser thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) {
        ...
        if (error == nil) {
            ...
            if ([objectId isEqualToString:user.username]) {
                if (data != nil) {
                    strongSelf.model.isDefaultAvatar = NO;
                    strongSelf.model.avatarDataLength = data.length;
                    [strongSelf.headView setImage:[UIImage imageWithData:data]];
                } else {
                    [strongSelf.headView setImage:[ImageBundle imagewithBundleName:@"headDefalt"]];
                }
            }
        } else {
