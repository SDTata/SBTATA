...
- (id)init {
    ...
    if (self != nil) {
        ...
        _imageViewAnntation.image = [ImageBundle imagewithBundleName:@"location_current"];
...
- (void)setMessageContentWith:(JMSGMessage *)message handler:(void (^)(NSUInteger))block {
    ...
    if (isReceived) {
        maskImage = [ImageBundle imagewithBundleName:@"chat_bg_other"];
        //发送地理位置要求背景色为白色
        if (message.contentType == kJMSGContentTypeLocation) {
            maskImage = [ImageBundle imagewithBundleName:@"chat_bg_location_other"];
        }
        _textContent.textColor = RGB_COLOR(@"#333333", 1);

    } else {
        maskImage = [ImageBundle imagewithBundleName:@"chat_bg_my"];
        if (message.contentType == kJMSGContentTypeLocation) {
            maskImage = [ImageBundle imagewithBundleName:@"chat_bg_location_my"];
        }
        _textContent.textColor = [UIColor whiteColor];
    ...
    switch (message.contentType) {
        case kJMSGContentTypeText:{
            ...
            for (NSTextCheckingResult *result in resultArr) {
                ...
                UIImage *emojiImage = [ImageBundle imagewithBundleName:imageName];
        ...
        case kJMSGContentTypeImage: {
            ...
            [(JMSGImageContent *)message.content thumbImageData:^(NSData *data, NSString *objectId, NSError *error) {
                ...
                if (error == nil) {
                    if (data != nil) {
                        [strongSelf setImage:[UIImage imageWithData:data]];
                    } else {
                        [strongSelf setImage:[ImageBundle imagewithBundleName:@"receiveFail"]];
                    }
                } else {
                    [strongSelf setImage:[ImageBundle imagewithBundleName:@"receiveFail"]];
                }
                if (block) {
                    NSData *imageData = UIImagePNGRepresentation(strongSelf.image);
        ...
        case kJMSGContentTypeVoice: {
            ...
            if (isReceived) {//height = 50
                ...
                [_voiceConent setImage:[ImageBundle imagewithBundleName:@"ReceiverVoiceNodePlaying"]];
            } else {
                [_voiceConent setFrame:CGRectMake(self.frame.size.width - 40, 15, 20, 20)];
                [_voiceConent setImage:[ImageBundle imagewithBundleName:@"SenderVoiceNodePlaying"]];
            }
...
//                    DDLogDebug(@"get message voiceData fail with error %@",error);
        ...
        case kJMSGContentTypeFile: {
            ...
            [self setImage:[ImageBundle imagewithBundleName:@"file_message_bg"]];
        ...
        case kJMSGContentTypeLocation: {
            ...
            self.contentMode = UIViewContentModeScaleAspectFit;
            //[self setImage:[ImageBundle imagewithBundleName:@"location_address"]];
            ...
            [_mapView takeSnapshotInRect:_mapView.bounds withCompletionBlock:^(UIImage *resultImage, CGRect rect) {
                ...
                [strongSelf.mapIV setImage:resultImage];
            }];
