...
- (void)updateMeters {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        strongSelf.currentTimeInterval = strongSelf.recorder.currentTime;
        
        if (!strongSelf->_isPause) {
            float progress = strongSelf.currentTimeInterval / strongSelf.maxRecordTime * 1.0;
            dispatch_async(dispatch_get_main_queue(), ^{
                if (strongSelf.recordProgress) {
                    strongSelf.recordProgress(progress);
                }
            });
        }
        
        float peakPower = [strongSelf.recorder averagePowerForChannel:0];
        double ALPHA = 0.015;
        double peakPowerForChannel = pow(10, (ALPHA * peakPower));
        
        dispatch_async(dispatch_get_main_queue(), ^{
            // 更新扬声器
            if (strongSelf.peakPowerForChannel) {
                strongSelf.peakPowerForChannel(peakPowerForChannel);
            }
        });
        
        if (strongSelf.currentTimeInterval > strongSelf.maxRecordTime) {
            [strongSelf stopRecord];
            dispatch_async(dispatch_get_main_queue(), ^{
                strongSelf.maxTimeStopRecorderCompletion();
            });
        }
    });
}
