      ...
      if (_photo.message.message.msgId) {
        ...
        [((JMSGImageContent *)message.content) largeImageDataWithProgress:^(float percent,NSString *msgId){
          STRONGSELF
            ...
            strongSelf->_photoLoadingView.progress = percent;
          ...
          if (error == nil) {
            ...
            strongSelf->_imageView.image = [UIImage imageWithData:data];
      ...
      if (_photo.message.message) {
        ...
        [((JMSGImageContent *)message.content) largeImageDataWithProgress:^(float percent,NSString *msgId){
          ...
          if (error == nil) {
            ...
            [strongSelf photoDidFinishLoadWithImage:strongPhoto.image];
            strongSelf->_imageView.image = strongPhoto.image;
            [strongSelf adjustFrame];
          } else {
            //[MBProgressHUD showMessage:@"下载大图失败!！" view:self];
              [MBProgressHUD showMessage:@"下载大图失败！" toView:self];
...
WeakSelf
        [UIView animateWithDuration:  0.3  animations:^{
            ...
            strongSelf.photo.srcImageView.image = strongSelf.photo.placeholder;
            [strongSelf photoStartLoad];
        }];
    } else {
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        ...
        strongSelf->_imageView.frame = [strongSelf.photo.srcImageView convertRect:strongSelf.photo.srcImageView.bounds toView:nil];
        
        // gif图片仅显示第0张
        if (strongSelf->_imageView.image.images) {
            strongSelf->_imageView.image = strongSelf->_imageView.image.images[0];
        }
        
        // 通知代理
        if ([strongSelf.photoViewDelegate respondsToSelector:@selector(photoViewSingleTap:)]) {
            [strongSelf.photoViewDelegate photoViewSingleTap:strongSelf];
        }
    } completion:^(BOOL finished) {
        STRONGSELF
        if (strongSelf == nil) {
        ...
        // 通知代理
        if ([strongSelf.photoViewDelegate respondsToSelector:@selector(photoViewDidEndZoom:)]) {
            [strongSelf.photoViewDelegate photoViewDidEndZoom:strongSelf];
        }
    }];
}
