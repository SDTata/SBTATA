...
- (void)setDataWithModel:(JCHATPhotoModel *)model withDelegate:(id)delegate{
  ...
  if (model.asset == nil) {
    ...
    [model.cachingManager requestImageForAsset:asset
        ...
        if ([strongSelf.thumbImageModel.photoAsset.localIdentifier isEqualToString:asset.localIdentifier]) {
                                     strongSelf.thumbImage.image = result;
                                   }
                                 }];
  } else {
    WeakSelf
    [[[ALAssetsLibrary alloc]init] assetForURL:model.imgURL resultBlock:^(ALAsset *asset)  {
      ...
      if (strongSelf == nil) {
        ...
        dispatch_main_async_safe(^{
            strongSelf.thumbImage.image = [UIImage imageWithCGImage:[asset thumbnail]];
        });
