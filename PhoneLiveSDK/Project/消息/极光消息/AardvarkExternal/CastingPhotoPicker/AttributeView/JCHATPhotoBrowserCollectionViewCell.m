...
- (void)tapPhotoAction:(UITapGestureRecognizer *)tap{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        strongSelf.largeImage.transform = CGAffineTransformIdentity;
        strongSelf.imageContent.contentSize = strongSelf.largeImage.frame.size;
        strongSelf.imageContent.contentOffset = CGPointZero;
    }];
  ...
  if (model.asset == nil) {
    ...
    WeakSelf
      [model.cachingManager requestImageForAsset:asset
          ...
          if ([strongSelf.photoModel.photoAsset.localIdentifier isEqualToString:asset.localIdentifier]) {
                                     strongSelf.largeImage.image = result;
                                   }
                                 }];
  } else {
      WeakSelf
      [[[ALAssetsLibrary alloc] init] assetForURL:model.imgURL resultBlock:^(ALAsset *asset)  {
          ...
          dispatch_main_async_safe(^{
              strongSelf.largeImage.image = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]];
          });
