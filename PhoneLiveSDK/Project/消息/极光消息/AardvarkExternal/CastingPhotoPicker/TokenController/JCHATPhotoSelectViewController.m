          ...
          if (group != nil) {
            ...
            [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
              if (result != nil) {
                ...
                [strongSelf->allPhotoArr addObject:model];
                  });
              }
            }];
            // 将所有获取的 照片模型 交给manager统一管理
              dispatch_main_async_safe(^{
                  [strongSelf->photoGridView reloadData];
                  [strongSelf->photoGridView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:strongSelf->allPhotoArr.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
              });
          }
            ...
            dispatch_main_async_safe((^{
                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(YZMsg(@"提示"),nil) message:NSLocalizedString(@"请允许访问相册,方可使用此功能!\n您可以在\"设置->隐私->照片\"中启用",nil) delegate:strongSelf cancelButtonTitle:NSLocalizedString(YZMsg(@"取消"),nil) otherButtonTitles:NSLocalizedString(@"设置",nil), nil];
                [alertView show];
            }));
        }];
}
- (void)setUpCollectionView {
  ...
  [photoGridView registerNib:[UINib nibWithNibName:@"ThumbImageCollectionViewCell" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]] forCellWithReuseIdentifier:@"ThumbImageCollectionViewCell"];
...
- (IBAction)clickToBrowserSelectedPhotos:(id)sender {
    JCHATPhotoBrowserViewController *photoBrowserVC = [[JCHATPhotoBrowserViewController alloc] initWithNibName:@"JCHATPhotoBrowserViewController" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
...
- (void)didSelectStatusChange:(JCHATPhotoModel *)model {
  ...
  if ([[[UIDevice currentDevice]systemVersion] floatValue]>= 8) {
    ...
    if (selectedPhotoDic[model.asset] == nil) {
      if (selectedPhotoDic.count > 8) {
          [MBProgressHUD showMessage:@"最多选择9张图片" toView:[UIApplication sharedApplication].delegate.window];
        return;
      }
      [selectedPhotoDic setObject:model forKey:model.imgURL];
    } else {
