...
- (void)viewDidLoad {// add model
    ...
    [_albumTable registerNib:[UINib nibWithNibName:@"JCHATAlbumTableViewCell" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]]
...
- (void)prepareAlbumArrWithAssert {//ios 8 以前使用 AssetsLibrary
    ...
    void (^assetsGroupsEnumerationBlock)(ALAssetsGroup *, BOOL *) = ^(ALAssetsGroup *assetsGroup, BOOL *stop) {
        if(assetsGroup) {
            ...
            strongSelf.getDataschedule --;
            [assetsGroup setAssetsFilter:[ALAssetsFilter allPhotos]];
            if(assetsGroup.numberOfAssets > 0) {
                JCHATAlbumModel *model = [JCHATAlbumModel new];
                [model setDataWithAssets:assetsGroup];
                [strongSelf.albumArr addObject:model];
            }
            dispatch_main_async_safe(^{
            if (strongSelf.getDataschedule == 0) {
                [strongSelf.albumTable reloadData];//移出去
            }
            });
        }
    };
...
- (void)pushToSelectPhotoVCWithIndex:(NSInteger)index {
    JCHATPhotoSelectViewController *selectPhotoVC = [[JCHATPhotoSelectViewController alloc] initWithNibName:@"JCHATPhotoSelectViewController" bundle:[XBundle currentXibBundleWithResourceName:NSStringFromClass([self class])]];
