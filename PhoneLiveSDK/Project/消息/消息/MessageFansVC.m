...
-(void)pullData {
    NSString *url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"index.php?service=Message.fansLists&uid=%@&p=%d",[Config getOwnID],_paging];
    WeakSelf
    [[YBNetworking sharedManager] postNetworkWithUrl:url  withBaseDomian:NO andParameter:nil data:nil success:^(int code, NSArray *info, NSString *msg) {
        ...
        if (code == 0) {
            ...
            if (strongSelf->_paging==1) {
                [strongSelf.dateArray removeAllObjects];
            }
            if (infoA.count==0) {
                [strongSelf.tableView.mj_footer endRefreshingWithNoMoreData];
            }else{
                [strongSelf.dateArray addObjectsFromArray:infoA];
            }
            if (strongSelf.dateArray.count<=0) {
                [PublicView showTextNoData:strongSelf.tableView text1:@"" text2:@"你还没有收获粉丝"];
            }else {
