...
-(void)pullFollow {
    _pubUrl = [[DomainManager sharedInstance].domainGetString stringByAppendingFormat:@"?service=User.getFollowsList&uid=%@&touid=%@&p=%d&key=%@",[Config getOwnID],[Config getOwnID],_paging,_key];
    
    [self pullData];
}
-(void)pullSearch {
    _pubUrl = [[DomainManager sharedInstance].domainGetString stringByAppendingFormat:@"?service=Home.search&key=%@&uid=%@&token=%@&p=%d",_key,[Config getOwnID],[Config getOwnToken],_paging];
    [self pullData];
}
...
-(void)pullData {
    ...
    WeakSelf
    [[YBNetworking sharedManager] postNetworkWithUrl:_pubUrl   withBaseDomian:NO andParameter:nil data:nil success:^(int code, NSArray *info, NSString *msg) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
        ...
        if (code == 0) {
            ...
            if (strongSelf.dataArray.count<=0) {
                [PublicView showTextNoData:strongSelf.tableView text1:strongSelf->_noData1 text2:strongSelf->_noData2];
            }else {
...
-(UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            ...
            if ([strongSelf->_showType isEqual:@"2"]&&strongSelf->_key.length>0) {
                [strongSelf pullSearch];
            }else{
                [strongSelf pullFollow];
            }
        }];
...
- (UIView *)topMix {
    if (!_topMix) {
        ...
        [_closeBtn setImage:[ImageBundle imagewithBundleName:@"gray_close"] forState:0];
        ...
        if (@available(iOS 13.0, *)) {
            ...
            [textField setValue:GrayText forKeyPath:@"placeholderLabel.textColor"];
            [textField setValue:[UIFont systemFontOfSize:16] forKeyPath:@"placeholderLabel.font"];
        } else {
            textField = [_searchBar valueForKey:@"searchField"];
