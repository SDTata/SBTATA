...
+(MessageListCell*)cellWithTab:(UITableView *)tableView andIndexPath:(NSIndexPath*)indexPath {
    ...
    if (!cell) {
        cell = [[[XBundle currentXibBundleWithResourceName:@"MessageListCell"]loadNibNamed:@"MessageListCell" owner:nil options:nil]objectAtIndex:0];
...
//    }
    ...
    if ([_model.sex isEqual:@"1"]) {
        _sexI.image= [ ImageBundle imagewithBundleName:@"sex_man"];
    }else if ([_model.sex isEqual:@"2"]){
        _sexI.image = [ImageBundle imagewithBundleName:@"sex_woman"];
    }else{
        _sexI.image= [ ImageBundle imagewithBundleName:@"sex_woman"];
    }
