...
//    bgi.image = [self drawBckgroundImage];//[ImageBundle imagewithBundleName:@"login_navi"];
...
//    }
    ...
    [left_btn setImage:[ImageBundle imagewithBundleName:imgL] forState:0];
    if (_haveTitleL == YES) {
        //有标题就把图片隐藏
    ...
    if (isImg == YES) {//130.7 * 40
        ...
        midIV.image = [ImageBundle imagewithBundleName:mid];
    ...
    if (_haveImgR == YES) {
        [right_btn setImage:[ImageBundle imagewithBundleName:imgR] forState:0];
    }
    if (_haveTitleR == YES) {
