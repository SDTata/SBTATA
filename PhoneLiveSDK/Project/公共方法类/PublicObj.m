...
+(BOOL)checkNull:(NSString *)str {
    if (![str isKindOfClass:[NSString class]] || [str isEqual:@"<null>"]||[str isEqual:@"(null)"] ||[str isEqual:@"null"]||[str isKindOfClass:[NSNull class]]||str.length==0) {
        return YES;
    }
    return NO;
}
...
#pragma mark - 获取扑克图像
+(NSString*)getPokerNameBy:(NSString*)pokerName
{
    if ([PublicObj checkNull:pokerName] || pokerName.length<2) {
        return @"";
    }
    NSString *pointStr = minstr([pokerName substringFromIndex:2]);
    if ([pointStr isEqualToString:@"J"]) {
        pointStr = @"11";
    }else if ([pointStr isEqualToString:@"Q"]) {
        pointStr = @"12";
    }else if ([pointStr isEqualToString:@"K"]) {
        pointStr = @"13";
    }else if ([pointStr isEqualToString:@"A"]) {
        pointStr = @"1";
    }
    
    NSString *pokerTy = @"1";
    if ([pokerName rangeOfString:YZMsg(@"PublicObj_Spade")].location!=NSNotFound) {
        pokerTy = @"1";
    }else if ([pokerName rangeOfString:YZMsg(@"PublicObj_Heart")].location!=NSNotFound) {
        pokerTy = @"2";
    }else if ([pokerName rangeOfString:YZMsg(@"PublicObj_Club")].location!=NSNotFound) {
        pokerTy = @"3";
    }else if ([pokerName rangeOfString:YZMsg(@"PublicObj_Diamond")].location!=NSNotFound) {
        pokerTy = @"4";
    }
    NSString *pokerS = [NSString stringWithFormat:@"poker%@_%@",pointStr,pokerTy];
    
    return pokerS;
}

+(NSArray *)getPokersNamesBy:(NSString*)pokersName
{
    NSArray *resultsA  = [pokersName componentsSeparatedByString:@"|"];
    if (resultsA.count == 2) {
        NSString *blueStr = [resultsA.firstObject stringByReplacingOccurrencesOfString:YZMsg(@"PublicObj_blue") withString:@""];
        NSString *redStr = [resultsA.lastObject stringByReplacingOccurrencesOfString:YZMsg(@"PublicObj_red") withString:@""];
        if (blueStr.length>0 && redStr.length>0) {
            NSArray *blueAr = [blueStr componentsSeparatedByString:@","];
            NSArray *redAr = [redStr componentsSeparatedByString:@","];
            NSArray * arrayresult = [blueAr arrayByAddingObjectsFromArray:redAr];
...
+ (CGFloat)safeBottomHeight{
    if (@available(iOS 11.0, *)) {
        ...
        if (topWindow.safeAreaInsets.bottom > 0.0) {
            return 34.0;
        }
    }
    return 0;
...
+(NSString*)removeFloatAllZero:(NSString*)string{
    ...
    NSString *outNumber = [NSString stringWithFormat:@"%@",[[NSDecimalNumber decimalNumberWithString:[NSString stringWithFormat:@"%@",testNumber]]stringValue]];
    return outNumber;
}
@end
