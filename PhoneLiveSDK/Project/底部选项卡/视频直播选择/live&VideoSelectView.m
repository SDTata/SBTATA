...
- (void)creatUI{
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        if ([arr[i] isEqualToString:YZMsg(@"立即直播")]) {
            imgView.image = [ImageBundle imagewithBundleName:@"livenow_jinshan"];
        }else{
            imgView.image = [ImageBundle imagewithBundleName:@"takevideo_jinshan"];
        }
    ...
    [closeBtn setImage:[ImageBundle imagewithBundleName:@"jinshan_close"] forState:0];
...
- (void)showWhiteView{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        strongSelf->whiteView.frame = CGRectMake(0, _window_height - (150+ShowDiff), _window_width, 150+ShowDiff);
    }];
}
- (void)hidBtnClick{
    WeakSelf
    [UIView animateWithDuration:0.3 animations:^{
       ...
       if (strongSelf == nil) {
        ...
        strongSelf->whiteView.frame = CGRectMake(0, _window_height, _window_width, 150+ShowDiff);
    } completion:^(BOOL finished) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
        [strongSelf.delegate hideSelctView];
    }];
}
@end
