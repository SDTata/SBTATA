...
- (void)configureViewsLayoutWithButtonSize:(CGSize)centerButtonSize {
    ...
    //设置中间按钮
    _pathCenterButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.centerImage.size.width/2*0.75, self.centerImage.size.height/2*0.75)];
    ...
    _pathCenterButton.center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2 - 17);
    
    [self addSubview:_pathCenterButton];
}

- (void)setIcon:(UIImage *)icon {
    [_pathCenterButton setImage:icon forState:UIControlStateNormal];
}
...
- (void)centerButtonTapped {
    [self.delegate pathButton:(MXtabbar*)_pathCenterButton clickItemButtonAtIndex:0];
}
