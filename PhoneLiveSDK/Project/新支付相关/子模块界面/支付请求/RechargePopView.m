...
- (void)setupUI {
    ...
    [self.cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(view2.mas_bottom).offset(4);
...
- (UILabel *)titleLbl {
    if (!_titleLbl) {
        ...
        _titleLbl.text = YZMsg(@"RechargePop_Warning");
...
- (UILabel *)billingLab {
    if (!_billingLab) {
        ...
        _billingLab.text = YZMsg(@"RechargePop_bindPhoneTitle");
        _billingLab.textColor = RGB_COLOR(@"#333333", 1);
        _billingLab.font = [UIFont systemFontOfSize:15];
        _billingLab.numberOfLines = 2;
        _billingLab.textAlignment = NSTextAlignmentCenter;
        _billingLab.adjustsFontSizeToFitWidth = YES;
        _billingLab.minimumScaleFactor = 0.5;
    }
    return _billingLab;
}
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitle:YZMsg(@"publictool_sure") forState:UIControlStateNormal];
        ...
        _cancelBtn.titleLabel.numberOfLines = 2;
        [_cancelBtn setTitleEdgeInsets:UIEdgeInsetsMake(5, 5, 5, 5)];
        _cancelBtn.titleLabel.minimumScaleFactor = 0.3;
    }
    return _cancelBtn;
}
...
- (UIButton *)submitBtn {
    if (!_submitBtn) {
        ...
        [_submitBtn setTitle:YZMsg(@"RechargePop_ChargeContinue") forState:UIControlStateNormal];
        [_submitBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_submitBtn addTarget:self action:@selector(submitAction:) forControlEvents:UIControlEventTouchUpInside];
        _submitBtn.backgroundColor = RGB_COLOR(@"#FF34CD", 1);
        _submitBtn.layer.cornerRadius = 18;
        _submitBtn.layer.masksToBounds = YES;
        [_submitBtn setTitleEdgeInsets:UIEdgeInsetsMake(5, 5, 5, 5)];
        _submitBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
        _submitBtn.titleLabel.numberOfLines = 2;
        _submitBtn.titleLabel.minimumScaleFactor = 0.3;
    }
    return _submitBtn;
}
