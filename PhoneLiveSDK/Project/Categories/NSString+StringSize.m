...
-(NSArray *)catchArrayWithORC:(NSString*)lrcStr{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        if (tempStr.length > 8){
            ...
            NSString *str1 = minstr([tempStr substringWithRange:NSMakeRange(2, 1)]);
            NSString *str2 = minstr([tempStr substringWithRange:NSMakeRange(5, 1)]);
            if ([str1 isEqualToString:@":"] && [str2 isEqualToString:@"."]){
                NSString *biaoji=[array objectAtIndex:i];
                NSString *lrcStr=[NSString string];
                if (biaoji.length>9) {
                    lrcStr = minstr([[array objectAtIndex:i] substringWithRange:NSMakeRange(9, 1)]);
                } else {
                    lrcStr=@"";
                }
                NSString *timeStr = minstr([[array objectAtIndex:i] substringWithRange:NSMakeRange(0, 8)]);
                if (![minstr(lrcStr) isEqualToString:@"\\"]) {
                    [rootDic setObject:minstr(lrcStr) forKey:@"lrc"];
                }
                [rootDic setObject:minstr(timeStr) forKey:@"lrctime"];
