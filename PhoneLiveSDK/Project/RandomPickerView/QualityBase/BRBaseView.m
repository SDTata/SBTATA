...
- (UIView *)maskView1 {
    if (!_maskView1) {
        _maskView1 = [[UIView alloc]initWithFrame:SCREEN_BOUNDS];
        _maskView1.backgroundColor = self.pickerStyle.maskColor;
        // 设置子视图的大小随着父视图变化
        _maskView1.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        _maskView1.userInteractionEnabled = YES;
        UITapGestureRecognizer *myTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(didTapMaskView:)];
        [_maskView1 addGestureRecognizer:myTap];
    }
    return _maskView1;
}
...
- (void)removePickerFromView:(UIView *)view {
    if (view) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            CGFloat alertViewHeight = strongSelf.alertView.bounds.size.height;
            CGRect rect = strongSelf.alertView.frame;
            rect.origin.y += alertViewHeight;
            strongSelf.alertView.frame = rect;
            if (!strongSelf.pickerStyle.hiddenMaskView) {
                strongSelf.maskView1.alpha = 0;
            }
