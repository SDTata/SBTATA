...
- (void)handlerDefaultSelectDate {
    ...
    if (!self.selectDate) {
        if (self.selectValue && self.selectValue.length > 0) {
            NSDate *defaultSelDate = [minstr(self.selectValue) isEqualToString:minstr([self getNowString])] ? [NSDate date] : [NSDate br_getDate:self.selectValue format:self.selectDateFormatter];
            if (!defaultSelDate) {
    ...
    if (!self.selectValue || ![minstr(self.selectValue) isEqualToString:minstr([self getNowString])]) {
...
- (void)reloadDateArrayWithUpdateMonth:(BOOL)updateMonth updateDay:(BOOL)updateDay updateHour:(BOOL)updateHour updateMinute:(BOOL)updateMinute updateSecond:(BOOL)updateSecond {
    ...
    NSString *yearString = minstr(self.yearArr[self.yearIndex]);
    // 如果 yearString = 至今
    if (self.isAddToNow && [yearString isEqualToString:minstr([self getNowString])]) {
        self.monthArr = nil;
        self.dayArr = nil;
        self.hourArr = nil;
        self.minuteArr = nil;
    ...
    NSString *monthString = minstr(self.monthArr[self.monthIndex]);
    // 如果 monthString = 至今
    if (self.isAddToNow && [monthString isEqualToString:minstr([self getNowString])]) {
        self.dayArr = nil;
        self.hourArr = nil;
        self.minuteArr = nil;
        self.secondArr = nil;
    ...
    NSString *hourString = minstr(self.hourArr[self.hourIndex]);
    // 如果 hourString = 至今
    if (self.isAddToNow && [hourString isEqualToString:minstr([self getNowString])]) {
        self.minuteArr = nil;
        self.secondArr = nil;
    ...
    NSString *minuteString = minstr(self.minuteArr[self.minuteIndex]);
    // 如果 minuteString = 至今
    if (self.isAddToNow && [minuteString isEqualToString:minstr([self getNowString])]) {
        self.secondArr = nil;
        return;
    }
    if (updateSecond) {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if ([minstr(lastSelectValue) isEqualToString:[self getNowString]] && ![minstr(self.mSelectValue) isEqualToString:[self getNowString]]) {
...
- (void)reloadData {
    ...
    if (self.style == BRDatePickerStyleSystem) {
        ...
        if (self.selectValue && [minstr(self.selectValue) isEqualToString:[self getNowString]]) {
...
- (NSString *)getNowString {
    return minstr([NSBundle br_localizedStringForKey:@"至今" language:self.pickerStyle.language]);
}

- (NSString *)getYearText:(NSInteger)row {
    NSString *yearString = minstr(self.yearArr[row]);
    if ([yearString isEqualToString:[self getNowString]]) {
        return yearString;
    }
...
- (NSString *)getMonthText:(NSInteger)row {
    NSString *monthString = minstr(self.monthArr[row]);
    if ([monthString isEqualToString:[self getNowString]]) {
        return monthString;
    }
...
- (NSString *)getHourText:(NSInteger)row {
    NSString *hourString = minstr(self.hourArr[row]);
    if ([hourString isEqualToString:[self getNowString]]) {
        return hourString;
    }
