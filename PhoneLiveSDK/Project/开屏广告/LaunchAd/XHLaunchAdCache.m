...
+(void)clearDiskCache{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [strongSelf xhLaunchAdCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [strongSelf checkDirectory:[strongSelf xhLaunchAdCachePath]];
    });
}
...
+(void)clearDiskCacheWithImageUrlArray:(NSArray<NSURL *> *)imageUrlArray{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [imageUrlArray enumerateObjectsUsingBlock:^(NSURL * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if([strongSelf checkImageInCacheWithURL:obj]){
                [[NSFileManager defaultManager] removeItemAtPath:[strongSelf imagePathWithURL:obj] error:nil];
            }
        }];
    });
}
...
+(void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *allFilePaths = [strongSelf allFilePathWithDirectoryPath:[strongSelf xhLaunchAdCachePath]];
        NSArray *exceptImagePaths = [strongSelf filePathsWithFileUrlArray:exceptImageUrlArray videoType:NO];
...
+(void)clearDiskCacheWithVideoUrlArray:(NSArray<NSURL *> *)videoUrlArray{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [videoUrlArray enumerateObjectsUsingBlock:^(NSURL * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if([strongSelf checkVideoInCacheWithURL:obj]){
                [[NSFileManager defaultManager] removeItemAtPath:[strongSelf videoPathWithURL:obj] error:nil];
            }
        }];
    });
}
...
+(NSString *)videoNameWithURL:(NSURL *)url{
     return [[self md5String:url.path] stringByAppendingString:@".mp4"];
}

+(NSString *)keyWithURL:(NSURL *)url{
    return [self md5String:url.path];
}
