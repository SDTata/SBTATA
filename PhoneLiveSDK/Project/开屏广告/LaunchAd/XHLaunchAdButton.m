...
#define SkipTitle YZMsg(@"XHLaunchAdButton_titleSkip")
/** 倒计时单位 */
...
- (instancetype)initWithSkipType:(SkipType)skipType{
    ...
    if (self) {
        _skipType = skipType;
        CGFloat y = (XH_FULLSCREEN ? 44 : 20)+30;
        ...
        switch (skipType) {
            ...
            case SkipTypeRoundProgressText:{//环形
                self.frame = CGRectMake(XH_ScreenW-55,y, 82, 42);
            }
                break;
            default:
                break;
...
-(UILabel *)timeLab{
    if(_timeLab ==  nil){
        ...
        _timeLab.minimumScaleFactor = 0.3;
...
-(void)setLeftRightSpace:(CGFloat)leftRightSpace{
    ...
    CGFloat width = frame.size.width+leftRightSpace;
...
-(void)setTopBottomSpace:(CGFloat)topBottomSpace{
    ...
    CGRect frame = self.timeLab.frame;
    CGFloat height = frame.size.height+topBottomSpace;
