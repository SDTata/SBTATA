...
+ (void)setupFUSDK {
    ...
    // 设置人脸算法质量
    [FUAIKit shareKit].faceProcessorFaceLandmarkQuality = [FURenderKit devicePerformanceLevel] >1 ? FUFaceProcessorFaceLandmarkQualityHigh : FUFaceProcessorFaceLandmarkQualityMedium;
    
    // 设置小脸检测是否打开
    [FUAIKit shareKit].faceProcessorDetectSmallFace = [FURenderKit devicePerformanceLevel] > 1;
...
- (void)hideDemoView {
    ...
    [self.skinView saveBeautyModel];
    [self.shapeView saveBeautyModel];
    [self.filterView saveBeautyModel];
    [self.makeupView saveBeautyModel];
}
...
- (FUSegmentBar *)bottomBar {
    if (!_bottomBar) {
        _bottomBar = [[FUSegmentBar alloc] initWithFrame:CGRectMake(0, self.demoOriginY, CGRectGetWidth(self.targetView.bounds), 49) titles:@[
            FUMsg(@"美肤"),
            FUMsg(@"美型"),
            FUMsg(@"滤镜"),
//            FUMsg(@"贴纸"),
            FUMsg(@"美妆"),
//            FUMsg(@"美体")
        ] configuration:[FUSegmentBarConfigurations new]];
...
+ (void)updateBeautyBlurEffect {
    ...
    if ([FURenderKit devicePerformanceLevel] > 1) {
        // 根据人脸置信度设置不同磨皮效果
...
+ (void)loadDefaultBeauty {
    ...
    // 尝试加载之前保存的美颜参数
    [self loadSavedBeautyParameters];
}

/// 加载保存的美颜参数
+ (void)loadSavedBeautyParameters {
    // 加载美肤参数
    FUBeautySkinViewModel *skinViewModel = [FUBeautySkinViewModel loadCacheForKey:@"FUBeautySkinViewModel"];
    if (skinViewModel) {
        [skinViewModel setAllSkinValues];
    }
    
    // 加载美型参数
    FUBeautyShapeViewModel *shapeViewModel = [FUBeautyShapeViewModel loadCacheForKey:@"FUBeautyShapeViewModel"];
    if (shapeViewModel) {
        [shapeViewModel setAllShapeValues];
    }
    
    // 加载滤镜参数
    FUBeautyFilterViewModel *filterViewModel = [FUBeautyFilterViewModel loadCacheForKey:@"FUBeautyFilterViewModel"];
    if (filterViewModel) {
        [filterViewModel setFilterLevel];
    }
    
    // 加载美妆参数
    FUMakeupViewModel *makeupViewModel = [FUMakeupViewModel loadCacheForKey:@"FUMakeupViewModel"];
    if (makeupViewModel && makeupViewModel.selectedItem) {
        [makeupViewModel setMakeupPackage];
    }
}
