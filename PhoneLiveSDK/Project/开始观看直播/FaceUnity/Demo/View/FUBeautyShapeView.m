...
- (instancetype)initWithFrame:(CGRect)frame {
    FUBeautyShapeViewModel *viewModel = [FUBeautyShapeViewModel loadCacheForKey:@"FUBeautyShapeViewModel"];
    if (!viewModel) {
        viewModel = [[FUBeautyShapeViewModel alloc] init];
    }
    return [self initWithFrame:frame viewModel:viewModel];
}
...
- (void)saveBeautyModel {
    [FUBeautyShapeViewModel saveCache:self.viewModel forKey:@"FUBeautyShapeViewModel"];
}
...
- (void)recoverAction {
    [FUAlertManager showAlertWithTitle:nil message:FUMsg(@"是否将所有参数恢复到默认值") cancel:FUMsg(@"取消") confirm:FUMsg(@"确定") inController:nil confirmHandler:^{
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = FUMsg(shape.name);
    ...
    if (shape.differentiateDevicePerformance) {
        cell.disabled = self.viewModel.performanceLevel >1;
    } else {
        cell.disabled = NO;
    }
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    FUBeautyShapeModel *shape = self.viewModel.beautyShapes[indexPath.item];
    if (shape.differentiateDevicePerformance) {
        if (self.viewModel.performanceLevel <= 1) {
            NSString *tipmsg = [NSString stringWithFormat:@"%@",FUMsg(@"该功能只支持在高端机上使用")];
            [MBProgressHUD showError:tipmsg];
            [self.shapeCollectionView reloadData];
            if (self.viewModel.selectedIndex >= 0) {
                [self.shapeCollectionView selectItemAtIndexPath:[NSIndexPath indexPathForItem:self.viewModel.selectedIndex inSection:0] animated:NO scrollPosition:UICollectionViewScrollPositionNone];
            }
...
- (FUSquareButton *)recoverButton {
    if (!_recoverButton) {
        ...
        [_recoverButton setTitle:FUMsg(@"恢复") forState:UIControlStateNormal];
