...
- (void)encodeWithCoder:(NSCoder *)coder {
    [coder encodeObject:self.combinationMakeups forKey:@"combinationMakeups"];
...
- (instancetype)initWithCoder:(NSCoder *)coder {
    self = [super init];
    if (self) {
        _combinationMakeups = [coder decodeObjectForKey:@"combinationMakeups"];
...
- (void)setMakeupPackage {
    if (self.selectedIndex > 0) {
        [self selectCombinationMakeupAtIndex:self.selectedIndex];
    }
}

- (FUMakeupModel *)selectedItem {
    if (self.selectedIndex <= 0 || self.selectedIndex >= self.combinationMakeups.count) {
        return nil;
    }
    return self.combinationMakeups[self.selectedIndex];
}

+ (NSDictionary *)mj_objectClassInArray {
    return @{
        @"combinationMakeups": [FUMakeupModel class]
    };
}
...
- (void)selectCombinationMakeupAtIndex:(NSInteger)index {
    ...
    if (![FURenderKit shareRenderKit].makeup) {
        ...
        // 高端机打开全脸分割
        makeup.makeupSegmentation = [FURenderKit devicePerformanceLevel] > 1;
        [FURenderKit shareRenderKit].makeup = makeup;
    }
    FUMakeupModel *model = self.combinationMakeups[index];
    NSString *bundlePath = [[XBundle mainBundle] pathForResource:model.bundleName ofType:@"bundle"];
