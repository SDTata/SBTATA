...
- (void)setModel:(redListModel *)model{
    ...
    [_iconImgView sd_setImageWithURL:[NSURL URLWithString:_model.avatar_thumb] placeholderImage:[ImageBundle imagewithBundleName:@"iconShortVideoDefaultAvatar"]];
    _nameL.text = _model.user_nicename;
    if ([_model.type isEqual:@"1"]) {
        _contenL.text = YZMsg(@"redListCell_delayRedBag");
    }else{
        _contenL.text = YZMsg(@"redListCell_nowRedBag");
    }
    if ([_model.time isEqual:@"qiangguole"]) {
        [_qiangBtn.layer removeAllAnimations];
        _timeL.text = YZMsg(@"redListCell_GetNow");
