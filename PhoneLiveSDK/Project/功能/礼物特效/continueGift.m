...
//    NSString *giftcount = @"1";
    ...
    animationImgView.image = [ImageBundle imagewithBundleName:@"gift_anima1"];
    ...
    giftBGView.image = [ImageBundle imagewithBundleName:@"giftBG"];
    ...
    //用户    显示头像
    UIImage *headerImg = [ImageBundle imagewithBundleName:@"iconShortVideoDefaultAvatar"];
...
//        make.top.equalTo(giftBGView).offset(-15);
    ...
    if ([giftcount isEqual:@"1"]) {
        ...
        giftXL.text = [NSString stringWithFormat:@"%@  X",YZMsg(@"continueGift_continue")];
    }
...
//        [giftXImage layoutIfNeeded];
        ...
        animationImgView.image = [ImageBundle imagewithBundleName:@"gift_anima2"];
    ...
    if(_GiftPosition == 1)
    {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)),dispatch_get_main_queue(), ^{
            ...
            [strongSelf performSelectorOnMainThread:@selector(hideGiftPop1:) withObject:GiftPopView  waitUntilDone:NO];
        });
    }
    else
    {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)),dispatch_get_main_queue(), ^{
            ...
            [strongSelf performSelectorOnMainThread:@selector(hideGiftPop2:) withObject:GiftPopView  waitUntilDone:NO];
        });
    }
}
-(void)startGiftTimer
    ...
    if (_popListItem2 != 0) {
        ...
        else
        {
            ...
            [UIView animateWithDuration:0.8 animations:^{
                ...
                if (GiftPopView.frame.origin.y<= strongSelf.newheight) {
                    //移除一级弹出
                    strongSelf.popListItem1 = 0;
                    if(strongSelf.GiftPosition == 3)
                        strongSelf.GiftPosition = 2;//如果现在上下都有则设置成仅下有
                    else strongSelf.GiftPosition = 0;                   //否则设置成全无
                }
                else
                {
                    strongSelf.popListItem2 = 0;
                    //移除二级弹出
                    if(strongSelf.GiftPosition == 3)   strongSelf.GiftPosition = 1;//如果现在上下都有则设置成仅上有
                    else strongSelf.GiftPosition = 0;                   //否则设置成全无
                }
            }];
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.8 * NSEC_PER_SEC)),dispatch_get_main_queue(), ^{
                ...
                [strongSelf performSelectorOnMainThread:@selector(removeGiftPop:) withObject:GiftPopView  waitUntilDone:NO];
            });
