...
-(instancetype)initWithIcons:(NSString *)icons andName:(NSString *)name{
    ...
    if (self) {
        ...
        for (int j=0; j<5; j++) {
            ...
            NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
...
-(void)donghua{
    ...
    [UIView animateWithDuration:1.0 animations:^{
        ...
        strongSelf->buttomView.frame = CGRectMake(_window_width/3,_window_height*0.1,carWidth,carHeight);
    }];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [strongSelf->carImage1 removeFromSuperview];
        strongSelf->carImage2.hidden = NO;
        strongSelf->array2=[NSMutableArray array];
        for (int j=0; j<10; j++) {
            NSString *imageName=[NSString stringWithFormat:@"car--%d.png",j+1];
            NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
            UIImage *image=[UIImage imageWithContentsOfFile:path];
            [strongSelf->array2 addObject:image];
        }
        //要展示的动画
        strongSelf->carImage2.animationImages=strongSelf->array2;
        //一次动画的时间
        strongSelf->carImage2.animationDuration=[strongSelf->array2 count]*0.02;
        //只执行一次动画
        strongSelf->carImage2.animationRepeatCount = 3;
        //开始动画
        [strongSelf->carImage2 startAnimating];
        strongSelf->carImage3.hidden = NO;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.9 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
       ...
       if (strongSelf == nil) {
        ...
        [strongSelf->carImage2 removeFromSuperview];
        strongSelf->array3=[NSMutableArray array];
        for (int j=0; j<5; j++) {
            NSString *imageName=[NSString stringWithFormat:@"car--%d.png",j+1];
            NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
            UIImage *image=[UIImage imageWithContentsOfFile:path];
            [strongSelf->array3 addObject:image];
        }
        //要展示的动画
        strongSelf->carImage3.animationImages=strongSelf->array3;
        //一次动画的时间
        strongSelf->carImage3.animationDuration=[strongSelf->array3 count]*0.008;
        //只执行一次动画
        strongSelf->carImage3.animationRepeatCount =MAXFLOAT;
        //开始动画
        [strongSelf->carImage3 startAnimating];
        [UIView animateWithDuration:2.5 animations:^{
            strongSelf->buttomView.frame = CGRectMake(-_window_width,_window_height*0.15,carWidth,carHeight);
        }];
    });
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
       ...
       if (strongSelf == nil) {
        ...
        [strongSelf->carImage3 removeFromSuperview];
        strongSelf->carImage4.hidden = NO;
        for (int j=0; j<5; j++) {
            NSString *imageName=[NSString stringWithFormat:@"car_%d.png",j+1];
            NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
            UIImage *image=[UIImage imageWithContentsOfFile:path];
            [strongSelf->array4 addObject:image];
        }
        //要展示的动画
        strongSelf->carImage4.animationImages=strongSelf->array4;
        //一次动画的时间
        strongSelf->carImage4.animationDuration=[strongSelf->array4 count]*0.008;
        //只执行一次动画
        strongSelf->carImage4.animationRepeatCount = MAXFLOAT;
        //开始动画
        [strongSelf->carImage4 startAnimating];
        strongSelf->buttomView.frame = CGRectMake(_window_width, _window_height*0.4, carWidth, carHeight);
        
        [UIView animateWithDuration:2.0 animations:^{
            if (strongSelf == nil) {
                return;
            }
            strongSelf->buttomView.frame = CGRectMake(_window_width/5,_window_height*0.2,carWidth,carHeight);
        }];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
           if (strongSelf == nil) {
            ...
            [strongSelf->carImage4 removeFromSuperview];
            strongSelf->carImage5.hidden = NO;
            for (int j=0; j<10; j++) {
                NSString *imageName=[NSString stringWithFormat:@"car_%d.png",j+1];
                NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
                UIImage *image=[UIImage imageWithContentsOfFile:path];
                [strongSelf->array5 addObject:image];
            }
            //要展示的动画
            strongSelf->carImage5.animationImages=strongSelf->array5;
            //一次动画的时间
            strongSelf->carImage5.animationDuration=[strongSelf->array5 count]*0.02;
            //只执行一次动画
            strongSelf->carImage5.animationRepeatCount = 3;
            //开始动画
            [strongSelf->carImage5 startAnimating];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (strongSelf == nil) {
                    return;
                }
                strongSelf->carImage6.hidden = NO;
                [strongSelf->carImage5 removeFromSuperview];
                for (int j=0; j<5; j++) {
                    NSString *imageName=[NSString stringWithFormat:@"backCar%d.png",j+1];
                    NSString *path = [XResourceBundle pathForResource:imageName ofType:nil];
                    UIImage *image=[UIImage imageWithContentsOfFile:path];
                    [strongSelf->array6 addObject:image];
                }
                //要展示的动画
                strongSelf->carImage6.animationImages=strongSelf->array6;
                //一次动画的时间
                strongSelf->carImage6.animationDuration=[strongSelf->array6 count]*0.008;
                //只执行一次动画
                strongSelf->carImage6.animationRepeatCount = MAXFLOAT;
                //开始动画
                [strongSelf->carImage6 startAnimating];
                [UIView animateWithDuration:1.5 animations:^{
                    strongSelf->buttomView.frame = CGRectMake(-carWidth,-carHeight,carWidth,carHeight);
                }];
            });
        });
    });
