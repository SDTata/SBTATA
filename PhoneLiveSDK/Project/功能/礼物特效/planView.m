...
-(instancetype)initWithicon:(NSString *)icons andName:(NSString *)name{
    ...
    if (self) {
        ...
        showImage.image= [ImageBundle imagewithBundleName:@"plane_shadow.png"];
...
-(void)dodonghua{
    ...
    [UIView animateWithDuration:1.0 animations:^{
        ...
        strongSelf->buttomView .frame=CGRectMake(0,0,planW,planH);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        strongSelf->timerLabel =  [NSTimer scheduledTimerWithTimeInterval:0.05 target:strongSelf selector:@selector(random) userInfo:nil repeats:YES];
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (strongSelf == nil) {
                return;
            }
            [strongSelf->timerLabel invalidate];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.7 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:2.0 animations:^{
                ...
                strongSelf->buttomView.frame=CGRectMake(-_window_width,0,planW,planH);
            }];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (strongSelf == nil) {
                    return;
                }
                [strongSelf->buttomView removeFromSuperview];
            });
