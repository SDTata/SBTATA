...
- (void)viewDidLoad {
    ...
    [btn sizeToFit];
    [btn setTitle:YZMsg(@"public_cancel") forState:UIControlStateNormal];
    ...
    imageView.image = [ImageBundle imagewithBundleName:@"default_search.png"];
    imageView.contentMode = UIViewContentModeScaleAspectFit;
    imageView.hidden = YES;
    [self.view addSubview:imageView];
    if (@available(iOS 13.0, *)) {
        UITextField *textField =  self.search.searchTextField;
        
        [textField setValue:normalColors forKeyPath:@"placeholderLabel.textColor"];
        [textField setValue:[UIFont systemFontOfSize:10] forKeyPath:@"placeholderLabel.font"];
    } else {
        if (@available(iOS 13.0, *)) {
            UITextField *textField = self.search.searchTextField;
            [textField setValue:normalColors forKeyPath:@"placeholderLabel.textColor"];
...
//    hunyinCell *cell = [tableView dequeueReusableCellWithIdentifier:@"music"];
    ...
    if(cell ==nil) {
        cell = [[[XBundle currentXibBundleWithResourceName:@"hunyinCell"] loadNibNamed:@"hunyinCell" owner:nil options:nil] lastObject];
    }
    ...
    if ([manager fileExistsAtPath:loadPath]) {
        ...
        [[YBNetworking sharedManager] postNetworkWithUrl:url withBaseDomian:YES andParameter:nil data:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            ...
            if (code == 0) {
                ...
                [strongSelf saveLRC:lrcLink and:dic];
                if (path.length>0 && ![strongSelf.downloadingArray containsObject:path]) {
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        cell.isDown = YES;
                        [cell musicDownLoad];
                        [strongSelf.downloadingArray addObject:path];
                    });
                }else{
                    [MBProgressHUD showError:YZMsg(@"musicView_downloading")];
                }
...
//获取歌词
-(BOOL)saveLRC:(NSString *)LRC and:(NSDictionary *)dic{
