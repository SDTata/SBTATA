...
@property(nonatomic,strong)UIView *maskview1;// 遮罩
...
-(instancetype)initWithFrame:(CGRect)frame andsureblock:(axinblock)sureblock andcancleblock:(axinblock)cancleblock{
    ...
    if (self) {
        ...
        _maskview1 = [[UIView alloc]initWithFrame:CGRectMake(0,0,_window_width, _window_height)];
        [self addSubview:_maskview1];
        ...
        title.text = YZMsg(@"coastselectview_coast_money");
        [VC addSubview:title];
        ...
        NSArray *titles = @[YZMsg(@"coastselectview_Choose_to_charge_for"),YZMsg(@"public_cancel"),YZMsg(@"publictool_sure")];
        
        for (int i = 0; i<3; i++) {
            UIButton *btn = [UIButton buttonWithType:0];
            btn.titleLabel.font = fontThin(13);
            if (i == 0) {
                [btn setImage:[ImageBundle imagewithBundleName:YZMsg(@"docoasessssimage")] forState:UIControlStateNormal];
...
-(void)doactions:(UIButton *)sender{
    if ([sender.titleLabel.text isEqual:YZMsg(@"publictool_sure")]) {
    ...
    if ([sender.titleLabel.text isEqual:YZMsg(@"public_cancel")]) {
        self.cancleblock(@"");
    }
}
