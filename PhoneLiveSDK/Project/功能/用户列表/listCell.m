...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        _imageV = [[SDAnimatedImageView alloc]initWithFrame:CGRectMake(6.2,10.6,27.6,27.6)];
...
-(void)setModel:(listModel *)model{
    _model = model;
    [_imageV sd_setImageWithURL:[NSURL URLWithString:_model.iconName] placeholderImage:[ImageBundle imagewithBundleName:@"iconShortVideoDefaultAvatar"]];
    if ([_model.guard_type isEqual:@"0"]) {
        ...
        _levelimage.image = [ImageBundle imagewithBundleName:YZMsg(@"guardListCell_guard_low")];
    }else if ([_model.guard_type isEqual:@"2"]){
        _levelimage.image = [ImageBundle imagewithBundleName:YZMsg(@"guardListCell_guard_heig")];
    }
}
