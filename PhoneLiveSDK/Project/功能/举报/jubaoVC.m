...
- (void)requetData{
    ...
    NSString *url;
    if (_isLive) {
        url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=Live.getReportClass"];
    }else{
        url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=Video.getReportContentlist"];
    }
    WeakSelf;
    [[YBNetworking sharedManager] postNetworkWithUrl:url withBaseDomian:NO andParameter:nil data:nil success:^(int code, NSArray *info, NSString *msg) {
        ...
        if (code == 0) {
            strongSelf.dataArr = [NSMutableArray array];
            if (info) {
                strongSelf.dataArr = [NSMutableArray arrayWithArray:info];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (!headerLabel) {
        ...
        headerLabel.text = YZMsg(@"jubaoVC_headerLabel");
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    label.text = YZMsg(@"jubaoVC_Option_title");
...
//    placeLabel.text = @"输入举报理由";
    ...
    [btn setTitle:YZMsg(@"jubaoVC_submit") forState:0];
...
- (void)dojubao{
    ...
    if (selctCount == 100000) {
        [MBProgressHUD showError:YZMsg(@"jubaoVC_reason_title")];
        return;
    }
    ...
    if (_isLive) {
        url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=Live.setReport"];
        parameterDic = @{
                                   @"uid":[Config getOwnID],
                                   @"touid":_dongtaiId,
        ...
        url = [[DomainManager sharedInstance].baseAPIString stringByAppendingFormat:@"?service=Video.report"];
        parameterDic = @{
                         @"uid":[Config getOwnID],
                         @"videoid":_dongtaiId,
                         @"token":[Config getOwnToken],
    ...
    WeakSelf
    [[YBNetworking sharedManager] postNetworkWithUrl:url withBaseDomian:NO andParameter:parameterDic data:nil success:^(int code, NSArray *info, NSString *msg) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
...
//        weakSelf.view.frame = CGRectMake(0.0f, -keyboardRect.size.height, _window_width, _window_height);
//    }];
}
...
//    navi.imgTitleSameR = YES;
    ...
    } andMidTitle:YZMsg(@"jubaoVC_title")];
