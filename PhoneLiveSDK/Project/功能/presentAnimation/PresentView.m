...
WeakSelf
    [UIView animateWithDuration:0.3 animations:^{
        ...
        strongSelf.frame = CGRectMake(0, self.frame.origin.y, self.frame.size.width, self.frame.size.height);
    } completion:^(BOOL finished) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
        [strongSelf shakeNumberLabel];
    }];
...
- (void)hidePresendView
{
    ...
    [UIView animateWithDuration:0.30 delay:2 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        strongSelf.frame = CGRectMake(0, strongSelf.frame.origin.y - 20, strongSelf.frame.size.width, strongSelf.frame.size.height);
        strongSelf.alpha = 0;
    } completion:^(BOOL finished) {
        STRONGSELF
        if (strongSelf == nil) {
        ...
        if (strongSelf.completeBlock) {
            strongSelf.completeBlock(finished,strongSelf.animCount);
        }
        [strongSelf reset];
        strongSelf.finished = finished;
//        NSLog(@"%ld",_animCount);
...
//    _headImageView = [UIImageView ]  model.headImage;
    ...
    _giftLabel.text = [NSString stringWithFormat:@"%@%@",YZMsg(@"PresentView_send"),model.giftName];
