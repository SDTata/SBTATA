...
- (void)setupMainViewMask
{
    if (CGRectIsEmpty(self.bounds) || self.bounds.size.width == 0) {
        return;
    }

    CGFloat verticalExtension = 50;
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRect:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height + verticalExtension)];
...
- (void)setTitlesGroup:(NSArray *)titlesGroup
{
    ...
    if (self.titleType == SDCycleScrollViewTitleTypeOnly) {
        NSMutableArray *temp = [NSMutableArray new];
...
- (void)setupPageControl
{
    ...
    if (self.imagePathsGroup.count == 0 || self.titleType == SDCycleScrollViewTitleTypeOnly) return;
...
- (void)scrollToIndex:(int)targetIndex
{
    ...
    if (self.titleType == SDCycleScrollViewTitleTypeBottom) {
        [_mainView setContentOffset:CGPointMake(targetIndex * _flowLayout.itemSize.width, 0) animated:YES];
    }
}
...
- (void)layoutSubviews
{
    ...
    // billDebug: 更新 mainView 的 mask
    [self setupMainViewMask];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.titleType != SDCycleScrollViewTitleTypeOnly && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage];
        } else {
            UIImage *image = [ImageBundle imagewithBundleName:imagePath];
            if (!image) {
                image = [UIImage imageWithContentsOfFile:imagePath];
            }
            cell.imageView.image = image;
        }
    } else if (self.titleType != SDCycleScrollViewTitleTypeOnly && [imagePath isKindOfClass:[UIImage class]]) {
