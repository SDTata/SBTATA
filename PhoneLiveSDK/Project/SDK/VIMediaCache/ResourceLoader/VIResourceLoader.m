...
#pragma mark - Helper
- (NSData *)decryptDataIfNeeded:(NSData *)data forURL:(NSURL *)url {
    if ([url.pathExtension isEqualToString:@"aes"]) {
       
        return [FFAES decryptData:data key:KAESKEY];
    }
    return data;
}
...
- (void)startNoCacheWorkerWithRequest:(AVAssetResourceLoadingRequest *)request {
    ...
    requestWorker.dataProcessor = ^NSData *(NSData *data, NSURL *url) {
            return [self decryptDataIfNeeded:data forURL:url];
        };
