...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    NSURL *urlSSS = [[KTVHCURLTool tool] getCacheUrlKeyFromMd5Path:URLString];
    if ([URLString containsString:@".m3u"]) {
        KTVHCDataRequest *dataRequest = [[KTVHCDataRequest alloc] initWithURL:urlSSS headers:request.allHeaderFields];
        return [[KTVHCHTTPHLSResponse alloc] initWithConnection:self dataRequest:dataRequest];
    }
    NSString *tsPath = [[KTVHCURLTool tool] findTsURLFromTsPath:URLString];
    KTVHCDataRequest *dataRequest = [[KTVHCDataRequest alloc] initWithURL:[NSURL URLWithString:tsPath] headers:request.allHeaderFields];
