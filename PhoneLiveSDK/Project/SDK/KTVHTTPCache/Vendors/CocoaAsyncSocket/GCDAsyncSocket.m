...
- (void)closeWithError:(NSError *)error
{
	...
	#if TARGET_OS_IPHONE
	{
		if (readStream || writeStream)
		{
			...
			if (writeStream)
			{
				// 添加安全检查，确保 writeStream 是有效的
				if (CFGetTypeID(writeStream) == CFWriteStreamGetTypeID()) {
					CFWriteStreamSetClient(writeStream, kCFStreamEventNone, NULL, NULL);
					CFWriteStreamClose(writeStream);
					CFRelease(writeStream);
				} else {
					// writeStream 无效，直接设为 NULL 避免进一步使用
					NSLog(@"[GCDAsyncSocket] Warning: Invalid writeStream detected");
				}
				writeStream = NULL;
			}
		}
