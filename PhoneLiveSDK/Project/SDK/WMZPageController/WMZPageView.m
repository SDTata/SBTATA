...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        if (!pageVC.sonCanScroll &&
            ...
            UIPanGestureRecognizer *pan = pageVC.currentScroll.panGestureRecognizer;
            CGFloat velocity = [pan velocityInView:pageVC.currentScroll].y;
            if (velocity < 0) {
                pageVC.currentScroll.contentOffset = CGPointZero;
            } else if (pageVC.currentScroll.contentOffset.y > 0) {
//                pageVC.downSc.scrollEnabled = NO;
            } else {
//                pageVC.downSc.scrollEnabled = YES;
            }
        }
