...
- (nullable id<SDWebImageOperation>)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    return [self sd2_internalSetImageWithOriginalURL:url URL:url placeholderImage:placeholder options:options context:context setImageBlock:setImageBlock progress:progressBlock completed:completedBlock];
}
...
//-(NSString*)replaceUrl:(NSString*)urlStr domain:(NSString*)domainS
//{
//    NSString *replaceStr = [urlStr stringByReplacingOccurrencesOfString:@"http://" withString:@""];
//    NSString *replaceStr1 = [replaceStr stringByReplacingOccurrencesOfString:@"https://" withString:@""];
//    NSString *replaceStr2 = [replaceStr1 stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"%@/",domainS] withString:@""];
//    NSString *replaceStr3 = [replaceStr2 stringByReplacingOccurrencesOfString:domainS withString:@""];
//    return replaceStr3;
//    
//}

- (nullable id<SDWebImageOperation>)sd2_internalSetImageWithOriginalURL:(nullable NSURL *)originalURL
...
//    if ([CacheImageKey sharedManager].imageKey== nil) {
//        [CacheImageKey sharedManager].imageKey = [NSMutableDictionary dictionary];
//    }
//    
//    NSString *cacheUrlOriginalKey = originalURL.absoluteString;
//    
//    if (current_host_index>=[CacheImageKey sharedManager].cacheUrlHost.count) {
//        current_host_index = 0;
//    }
//    NSString *domainStr =@"";
//     
//    if ([originalURL isKindOfClass:NSString.class]) {
//        originalURL = [NSURL URLWithString:(NSString *)originalURL];
//    }
//    if (originalURL.host) {
//        domainStr = [self getDomainFromURL:originalURL];
//    }
//
//    NSString *keyStr = [self replaceUrl:cacheUrlOriginalKey domain:domainStr];
//    NSDictionary *subDic = [[CacheImageKey sharedManager].imageKey objectForKey:keyStr];
//    NSString *cacheUrl = [subDic objectForKey:@"url"];
//    
//    if (cacheUrl == nil || cacheUrl.length<5) {
//        if ([url isKindOfClass:NSString.class]) {
//            url = [NSURL URLWithString:(NSString *)url];
//        }
//        if (!url.host) {
//            //开始拼接域名
//            if ([CacheImageKey sharedManager].cacheUrlHost!= nil &&[CacheImageKey sharedManager].cacheUrlHost.count>0) {
//                NSString *stri =  [NSString stringWithFormat:@"%@/%@",[CacheImageKey sharedManager].cacheUrlHost[current_host_index],url];
//                url =[NSURL URLWithString:stri];
//             
//            }else{
//                url = url;
//            }
...
//        }else{
//            if ([CacheImageKey sharedManager].cacheUrlHost!= nil &&[CacheImageKey sharedManager].cacheUrlHost.count>0) {
//                NSString *stri =  [NSString stringWithFormat:@"%@/%@",[CacheImageKey sharedManager].cacheUrlHost[current_host_index],keyStr];
//                url =[NSURL URLWithString:stri];
//             
//            }else{
//                url = url;
...
//            NSString *domain = [self getDomainFromURL:imageURL];
//            if (error != nil) {
//                current_host_index++;
//                NSString *keyStr = [self replaceUrl:imageURL.absoluteString domain:domain];
//                NSDictionary *subDic = [[CacheImageKey sharedManager].imageKey objectForKey:keyStr];
//                NSString *errorCoun = [subDic objectForKey:@"errorCount"];
//                
//                [[CacheImageKey sharedManager].imageKey setObject:@{@"errorCount":minnum([errorCoun?:@"0" intValue]+1),@"url":@""} forKey:keyStr];
//                
//                if ([errorCoun?:@"0" intValue]<[CacheImageKey sharedManager].cacheUrlHost.count) {
//                    [self sd2_internalSetImageWithOriginalURL:originalURL URL:url placeholderImage:placeholder options:options context:context setImageBlock:setImageBlock progress:progressBlock completed:completedBlock];
//                }
//            }else{
//                [[CacheImageKey sharedManager].imageKey setObject:@{@"errorCount":@"0",@"url":imageURL} forKey:[self replaceUrl:imageURL.absoluteString domain:domain]];
//            }
        }];
