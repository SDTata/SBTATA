...
- (void)reloadData {
    ...
    dispatch_main_async_safe(^{
        ...
        if (weakSelf.isWaiting) {
            if (weakSelf.scrollTimer) {
                [weakSelf.scrollTimer invalidate];
                weakSelf.scrollTimer = nil;
            }
            [weakSelf resetAll];
            [weakSelf startAfterTimeInterval:YES];
        } else if (weakSelf.isScrolling) {
            [weakSelf resetAll];
        } else {
            // stopped
            [weakSelf resetAll];
...
- (void)resetAll {
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            if (i == 0) {
                [_items[index] setFrame:CGRectMake(0.0f, 0.0f, itemWidth, itemHeight)];
                lastMaxX = itemWidth;
...
- (void)scrollTimerDidFire:(NSTimer *)timer {
    ...
    dispatch_main_async_safe(^{
        ...
        if (strongSelf.direction == UUMarqueeViewDirectionLeftward) {
            [strongSelf moveToNextDataIndex];

            CGFloat itemHeight = CGRectGetHeight(strongSelf.frame);
            CGFloat firstItemWidth = CGRectGetWidth(strongSelf.frame);
            CGFloat currentItemWidth = CGRectGetWidth(strongSelf.frame);
            CGFloat lastItemWidth = CGRectGetWidth(strongSelf.frame);
            for (int i = 0; i < strongSelf.items.count; i++) {
                int index = (i + strongSelf.firstItemIndex) % strongSelf.items.count;

                CGFloat itemWidth = MAX(strongSelf.items[index].width + strongSelf.itemSpacing, CGRectGetWidth(strongSelf.frame));
            ...
            strongSelf.items[strongSelf.firstItemIndex].tag = strongSelf.dataIndex;
            strongSelf.items[strongSelf.firstItemIndex].width = [strongSelf itemWidthAtIndex:strongSelf.items[strongSelf.firstItemIndex].tag];
            CGFloat nextItemWidth = MAX(strongSelf.items[strongSelf.firstItemIndex].width + strongSelf.itemSpacing, CGRectGetWidth(strongSelf.frame));
            [strongSelf.items[strongSelf.firstItemIndex] setFrame:CGRectMake(lastItemWidth, 0.0f, nextItemWidth, itemHeight)];
            if (firstItemWidth != nextItemWidth) {
                // if the width of next item view changes, then recreate it by delegate
                [strongSelf.items[strongSelf.firstItemIndex] clear];
            }
            [strongSelf updateItemView:strongSelf.items[strongSelf.firstItemIndex] atIndex:strongSelf.items[strongSelf.firstItemIndex].tag];
            [UIView animateWithDuration:(currentItemWidth / strongSelf.scrollSpeed) delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
                if (strongSelf == nil) {
                    return;
                }
                CGFloat lastMaxX = 0.0f;
                for (int i = 0; i < strongSelf.items.count; i++) {
                    ...
                    if (i == 0) {
                        ...
                        [strongSelf.items[index] setFrame:CGRectMake(-itemWidth, 0.0f, itemWidth, itemHeight)];
                        lastMaxX = 0.0f;
                    } else {
                        [strongSelf.items[index] setFrame:CGRectMake(lastMaxX, 0.0f, itemWidth, itemHeight)];
                        lastMaxX = lastMaxX + itemWidth;
                    }
                }
                ...
                if (strongSelf) {
                    strongSelf.isScrolling = NO;
                    [strongSelf repeatWithAnimationFinished:finished];
                }
            }];
            [strongSelf moveToNextItemIndex];
        } else {
            [strongSelf moveToNextDataIndex];

            CGFloat itemWidth = CGRectGetWidth(strongSelf.frame);
            CGFloat itemHeight = CGRectGetHeight(strongSelf.frame) / strongSelf.visibleItemCount;

            // move the top item to bottom without animation
            strongSelf.items[strongSelf.firstItemIndex].tag = strongSelf.dataIndex;
            if (strongSelf.useDynamicHeight) {
                CGFloat firstItemWidth = strongSelf.items[strongSelf.firstItemIndex].height;
                strongSelf.items[strongSelf.firstItemIndex].height = [strongSelf itemHeightAtIndex:strongSelf.items[strongSelf.firstItemIndex].tag];
                [strongSelf.items[strongSelf.firstItemIndex] setFrame:CGRectMake(0.0f, CGRectGetMaxY(strongSelf.bounds), itemWidth, strongSelf.items[strongSelf.firstItemIndex].height)];
                if (firstItemWidth != strongSelf.items[strongSelf.firstItemIndex].height) {
                    // if the height of next item view changes, then recreate it by delegate
                    [strongSelf.items[strongSelf.firstItemIndex] clear];
                }
            } else {
                [strongSelf.items[strongSelf.firstItemIndex] setFrame:CGRectMake(0.0f, CGRectGetMaxY(strongSelf.bounds), itemWidth, itemHeight)];
            }
            [strongSelf updateItemView:strongSelf.items[strongSelf.firstItemIndex] atIndex:strongSelf.items[strongSelf.firstItemIndex].tag];

            if (strongSelf.useDynamicHeight) {
                int lastItemIndex = (int)(strongSelf.items.count - 1 + strongSelf.firstItemIndex) % strongSelf.items.count;
                CGFloat lastItemHeight = strongSelf.items[lastItemIndex].height;
                [UIView animateWithDuration:(lastItemHeight / strongSelf.scrollSpeed) delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
                    if (strongSelf == nil) {
                        return;
                    }
                    for (int i = 0; i < strongSelf.items.count; i++) {
                        ...
                        if (i == 0) {
                            ...
                            [strongSelf.items[index] setFrame:CGRectMake(CGRectGetMinX(strongSelf.items[index].frame),
                                                               CGRectGetMinY(strongSelf.items[index].frame) - lastItemHeight,
                                                               itemWidth,
                                                               strongSelf.items[index].height)];
                            strongSelf.items[index].alpha = 0.0f;
                        } else {
                            [strongSelf.items[index] setFrame:CGRectMake(CGRectGetMinX(strongSelf.items[index].frame),
                                                               CGRectGetMinY(strongSelf.items[index].frame) - lastItemHeight,
                                                               itemWidth,
                                                               strongSelf.items[index].height)];
                            strongSelf.items[index].alpha = 1.0f;
                        }
                    }
                } completion:^(BOOL finished) {
                    if (strongSelf == nil) {
                ...
                if (strongSelf.timeIntervalPerScroll <= 0.0) {
                    // smooth animation
                    animationOptions = UIViewAnimationOptionCurveLinear;
                }
                [UIView animateWithDuration:strongSelf.timeDurationPerScroll delay:0.0 options:animationOptions animations:^{
                    for (int i = 0; i < strongSelf.items.count; i++) {
                        int index = (i + strongSelf.firstItemIndex) % strongSelf.items.count;
                        if (i == 0) {
                            continue;
                        } else if (i == 1) {
                            [strongSelf.items[index] setFrame:CGRectMake(0.0f, -itemHeight, itemWidth, itemHeight)];
                        } else {
                            [strongSelf.items[index] setFrame:CGRectMake(0.0f, itemHeight * (i - 2), itemWidth, itemHeight)];
                        }
                    }
                } completion:^(BOOL finished) {
                    if (strongSelf == nil) {
