...
open class FoundationStream : NSObject, WSStream, StreamDelegate  {
    ...
    public func connect(url: URL, port: Int, timeout: TimeInterval, ssl: SSLSettings, completion: @escaping ((Error?) -> Void)) {
        ...
        #else
//        if DomainManager.sharedInstance().isOpenWebSocketProxy {
//            let proxySocksConfig = DomainManager.poxySocksDic() as CFDictionary
////            print(proxySocksConfig)
//            outputStream?.setProperty(proxySocksConfig, forKey: Stream.PropertyKey.socksProxyConfigurationKey)
//            inputStream?.setProperty(proxySocksConfig, forKey: Stream.PropertyKey.socksProxyConfigurationKey)
//        }
        #endif
