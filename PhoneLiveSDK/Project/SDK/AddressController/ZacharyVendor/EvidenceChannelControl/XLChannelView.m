...
-(void)dragEnd{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        strongSelf.dragingItem.frame = endFrame;
    }completion:^(BOOL finished) {
        STRONGSELF
        if (strongSelf == nil) {
            return;
        }
        strongSelf.dragingItem.hidden = true;
        XLChannelItem *item = (XLChannelItem*)[strongSelf.collectionView cellForItemAtIndexPath:strongSelf.dragingIndexPath];
        item.isMoving = false;
    }];
}
