...
- (void)updateLayout {
    ...
    if (_selectedIndex == _lastSelectedIndex && _selectedTitle == _lastSelectedTitle) {return;}
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [UIView performWithoutAnimation:^{
            ...
            if ([strongSelf.collectionView numberOfItemsInSection:0] == 0) {
                [strongSelf.collectionView reloadData];
            }else{
                if ([strongSelf.collectionView numberOfItemsInSection:0]>currentIndexPath.row) {
                    [strongSelf.collectionView reloadItemsAtIndexPaths:@[currentIndexPath]];
                }else{
                    [strongSelf.collectionView reloadData];
                }
    ...
    if (_lastSelectedIndex < [self.dataSource pageTitleViewNumberOfTitle]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [UIView performWithoutAnimation:^{
                ...
                if ([strongSelf.collectionView numberOfItemsInSection:0]>lastIndexPath.row) {
                    [strongSelf.collectionView reloadItemsAtIndexPaths:@[lastIndexPath]];
                }else{
                    [strongSelf.collectionView reloadData];
                }
    ...
    if (center.x <= 0) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [strongSelf fixShadowLineCenter];
        });
    }else {
...
- (void)fixShadowLineCenter {
    ...
    if (_selectedIndex >= [self.collectionView numberOfItemsInSection:0]) {
        _selectedIndex = [self.collectionView numberOfItemsInSection:0]-1;
    }
    ...
    if (shadowCenter.x > 0) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.01 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
            ...
            if (strongSelf == nil) {
                ...
                strongSelf.shadowLine.center = [strongSelf shadowLineCenterForIndex:strongSelf.selectedIndex];
            });
        }
    }
}
