...
- (void)displayTitleview:(BOOL)bShow{
    if(!bShow){
        self.config.titleViewHeight = 0;
        
        self.titleView.hidden = YES;
        self.titleView.height = self.config.titleViewHeight;
        CGRect f = self.pageVC.view.frame;
        f.origin.y = 0;
        self.pageVC.view.frame = f;
        ...
        self.titleView.height = self.config.titleViewHeight;
        CGRect f = self.pageVC.view.frame;
        f.origin.y = self.config.titleViewHeight;
        self.pageVC.view.frame = f;
...
- (BOOL)pageTitleViewDidSelectedAtIndex:(NSInteger)index {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(SetViewControllersMethodDelay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [strongSelf delegateSelectedAdIndex:index];
    });
    return true;
}
...
- (BOOL)switchToViewControllerAdIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    //目标切换位置标题
    NSString *targetTitle1 = [self titleForIndex:index];
    
    if (index == _selectedIndex && [_titleView.lastSelectedTitle isEqualToString:targetTitle1] && index >= 0 && self.pageVCDidLoad) {return NO;}
    //设置正在加载标记
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [weakSelf.pageVC setViewControllers:@[[weakSelf viewControllerForIndex:index]] direction:direction animated:animated completion:^(BOOL finished) {
            if (weakSelf == nil) {
                return;
            }
    ...
    //标题居中
    self.titleView.selectedTitle = targetTitle1;
    self.titleView.selectedIndex = _selectedIndex;
    self.titleView.lastSelectedTitle = targetTitle1;
    return YES;
}
...
- (UIViewController *)getVCByIndex:(NSInteger)index {
    ...
    for (UIViewController *vc in self.shownVCArr) {
        if ([minstr(vc.xl_title) isEqualToString:targetTitle]) {
            return vc;
        }
    }
    return nil;
...
- (UIViewController *)viewControllerForIndex:(NSInteger)index {
    ...
    UIViewController *currentVC = self.pageVC.viewControllers.firstObject;
    //当前标题
    NSString *currentTitle = minstr(currentVC.xl_title);
    //目标切换位置标题
    NSString *targetTitle = [self titleForIndex:index];
