...
- (NSTimeInterval)emas_freshnessLifetime {
    ...
    // ✅ 对于没有明确缓存头的响应，基于资源类型进行启发式缓存
    // 静态资源（CSS、JS、图片等）通常可以安全地缓存更长时间
    if (![self.response isKindOfClass:[NSHTTPURLResponse class]]) {
        return 0;
    }

    NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)self.response;
    NSString *urlPath = httpResponse.URL.path.lowercaseString;

    // CSS 文件：7天缓存
    if ([urlPath hasSuffix:@".css"]) {
        return 7 * 24 * 3600;  // 7 days
    }

    // JavaScript 文件：7天缓存
    if ([urlPath hasSuffix:@".js"]) {
        return 7 * 24 * 3600;  // 7 days
    }

    // 图片文件：30天缓存
    if ([urlPath hasSuffix:@".png"] ||
        [urlPath hasSuffix:@".jpg"] ||
        [urlPath hasSuffix:@".jpeg"] ||
        [urlPath hasSuffix:@".gif"] ||
        [urlPath hasSuffix:@".webp"] ||
        [urlPath hasSuffix:@".svg"]) {
        return 30 * 24 * 3600;  // 30 days
    }

    // 字体文件：30天缓存
    if ([urlPath hasSuffix:@".woff"] ||
        [urlPath hasSuffix:@".woff2"] ||
        [urlPath hasSuffix:@".ttf"] ||
        [urlPath hasSuffix:@".eot"]) {
        return 30 * 24 * 3600;  // 30 days
    }
