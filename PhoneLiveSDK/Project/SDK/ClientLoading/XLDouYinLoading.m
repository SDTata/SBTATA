...
//出现动画的时间
@property (nonatomic, strong) NSDate *aniShowTime;
//暂停动画的时间
@property (nonatomic, strong) NSDate *aniPauseTime;
...
+ (BOOL)hideInView:(UIView *)view {
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            ...
            @try {
                if(loading && !loading.isDisappear){
            ...
            @catch (NSException * e) {
                NSLog(@"Truncate file raise a exception: %@", e);
                return NO;
            }
        }
    }
...
- (void)updateBallAnimations {
    if(self.isPause){
        CGFloat timeDistance = [[NSDate date] timeIntervalSinceDate:self.aniPauseTime];
        if(timeDistance > PauseSecond){
            self.isPause = false;
        }else{
            return;
        }
    ...
    NSInteger timeDistance = [[NSDate date] timeIntervalSinceDate:self.aniShowTime];
    if(timeDistance > 3){
        [self stopAnimated];
