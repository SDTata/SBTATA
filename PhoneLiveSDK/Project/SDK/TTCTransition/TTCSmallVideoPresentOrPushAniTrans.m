...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    UIView *bigCurSnapshot = nil;
    if (self.bigCurPlayCell) {
        bigCurSnapshot = [self.smalCurPlayCell snapshotViewAfterScreenUpdates:NO];
    }
    
    [containerView addSubview:self.maskView];
    [containerView addSubview:self.toVC.view];
    [containerView addSubview:bigCurSnapshot];
    ...
    if(self.smalCurPlayCell && self.smalCurPlayCell.superview) {
        initialFrame = [self.smalCurPlayCell.superview convertRect:self.smalCurPlayCell.frame toView:self.toVC.view.superview];
    }
    ...
    bigCurSnapshot.frame = initialFrame;
    ...
    [UIView animateWithDuration:duration
        ...
        bigCurSnapshot.frame = CGRectMake(0, 0, CGRectGetWidth(self.bigCurPlayCell.frame), CGRectGetHeight(self.bigCurPlayCell.frame));
        
    } completion:^(BOOL finished) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [bigCurSnapshot removeFromSuperview];
