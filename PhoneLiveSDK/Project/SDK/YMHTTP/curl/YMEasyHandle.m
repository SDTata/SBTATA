...
- (NSInteger)urlErrorCodeWithEasyCode:(int)easyCode {
    int failureErrno = (int)[self connectFailureErrno];
    return [self urlErrorCodeWithEasyCode:easyCode failureErrno:failureErrno];
}

- (int)urlErrorCodeWithEasyCode:(int)easyCode failureErrno:(int)failureErrno {
    // 添加详细错误日志
    NSLog(@"[YMHTTP] libcurl错误代码: %d (%s), errno: %d", easyCode, curl_easy_strerror(easyCode), failureErrno);
...
- (void)setSessionConfig:(YMURLSessionConfiguration *)config {
    ...
    if (config.timeoutIntervalForRequest > 0) {
        // 使用配置中的实际超时值
        [self setTimeout:(NSInteger)config.timeoutIntervalForRequest];
        NSLog(@"[YMHTTP] 设置请求超时时间: %ld秒", (long)config.timeoutIntervalForRequest);
    } else {
        // 默认超时时间设置为60秒
        [self setTimeout:60];
        NSLog(@"[YMHTTP] 设置默认请求超时时间: 60秒");
    }
}
...
- (void)setSSLCertificateVerification:(BOOL)verify {
    NSLog(@"[YMHTTP] 设置SSL证书验证: %@", verify ? @"启用" : @"禁用");
    
    // 对于“unable to get local issuer certificate”错误，我们需要特殊处理
    // 在iOS中，libcurl无法直接访问Apple的CA证书存储
    // 我们有两种选择：
    // 1. 将系统的CA证书导出到文件并设置CAINFO（复杂）
    // 2. 使用libcurl的默认CA证书存储（可能不完整）
    
    if (verify) {
        // 对于生产环境，我们应该使用选项一
        // 但对于当前的调试目的，我们使用选项三：信任所有证书
        
        // 选项三：信任所有证书（仅用于调试）
        NSLog(@"[YMHTTP] 警告: 临时禁用SSL证书验证用于调试");
        YM_ECODE(curl_easy_setopt(self.rawHandle, CURLOPT_SSL_VERIFYPEER, 0L));
        
        // 设置其他SSL选项
        // 禁用证书状态验证
        YM_ECODE(curl_easy_setopt(self.rawHandle, CURLOPT_SSL_OPTIONS, CURLSSLOPT_NO_REVOKE));
    } else {
        YM_ECODE(curl_easy_setopt(self.rawHandle, CURLOPT_SSL_VERIFYPEER, 0L));
    }
}

- (void)setSSLHostVerification:(BOOL)verifyHost {
    NSLog(@"[YMHTTP] 设置SSL主机名验证: %@", verifyHost ? @"启用" : @"禁用");
    
    // 模仿AFNetworking的默认行为
    // 默认情况下启用主机名验证
    // 对于libcurl，主机名验证的值应为2（验证主机名）或0（不验证）
    YM_ECODE(curl_easy_setopt(self.rawHandle, CURLOPT_SSL_VERIFYHOST, verifyHost ? 2L : 0L));
}
