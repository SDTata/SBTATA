...
- (void)seekToTimeWhenRecordPlayback:(CMTime)time {
    ...
    [self.playerModel.player seekToTime:time completionHandler:^(BOOL finished) {
        ...
        if(finished){
            [strongSelf internalResumeWithNeedCallDelegate:YES];
        }
...
- (void)seekToHeaderThenStartPlayback {
    ...
    [self.playerModel.player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
        ...
        strongSelf.playerModel.lastTime = 0;
        [strong_Item.player play];
        [strongSelf callPlayerStatusDidChangeDelegateMethod];
...
- (void)callDelegateMethodWithError:(NSError *)error {
    ...
    JPDispatchSyncOnMainQueue(^{
        ...
        if (strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(videoPlayer:playFailedWithError:)]) {
            [strongSelf.delegate videoPlayer:strongSelf playFailedWithError:error];
        }
    });
}
