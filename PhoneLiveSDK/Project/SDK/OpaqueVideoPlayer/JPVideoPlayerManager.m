 ...
 configurationCompletion:(JPPlayVideoConfiguration)configurationCompletion {
    ...
    if (isFileURL) {
        ...
        [self.videoCache queryCacheOperationForKey:key completion:^(NSString *_Nullable videoPath, JPVideoPlayerCacheType cacheType) {
            ...
            if (!videoPath && (![strongSelf.delegate respondsToSelector:@selector(videoPlayerManager:shouldDownloadVideoForURL:)] || [strongSelf.delegate videoPlayerManager:self shouldDownloadVideoForURL:url])) {
                // play web video.
                JPDebugLog(@"Start play a web video: %@", url);
                strongSelf.managerModel.cacheType = JPVideoPlayerCacheTypeNone;
                [strongSelf.videoPlayer playVideoWithURL:url
                                           options:options
                                         showLayer:showLayer
                           configurationCompletion:configurationCompletion];
            }
            else if (videoPath) {
                strongSelf.managerModel.cacheType = JPVideoPlayerCacheTypeExisted;
                JPDebugLog(@"Start play a existed video: %@", url);
                [strongSelf playFragmentVideoWithURL:url
                                       options:options
                                     showLayer:showLayer
                       configurationCompletion:configurationCompletion];
            }
                ...
                [strongSelf callDownloadDelegateMethodWithFragmentRanges:nil
                                                      expectedSize:1
                                                         cacheType:JPVideoPlayerCacheTypeNone
                                                             error:error];
                [strongSelf reset];
